...
test('Watch companion presents safe pick context and routes commands through iPhone', () => {
  ...
  assert.match(watch, /model\.send\(\.readInstruction\)/)
  ...
  assert.match(app, /watch\.publish\(await picking\.makeWatchSnapshot\(\)\)/)
...
test('picker audio routing and Meta scan feedback stay explicit and bounded', () => {
  ...
  assert.match(app, /await voice\.speakAndWait\("Item matched\. Say confirm pick to submit\."\)/)
  assert.match(app, /await listenForConfirmation\(automatic: true\)/)
  assert.match(app, /timeout: automatic \? \.seconds\(8\) : nil/)
  assert.match(app, /voice\.speak\("Picks confirmed\."\)[\s\S]*await loadQueue\(readAloud: false\)/)
