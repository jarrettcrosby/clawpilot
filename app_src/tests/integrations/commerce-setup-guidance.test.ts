...
test('Shopify setup keeps its app-version scope list copyable', () => {
  ...
  assert.ok(result?.scopes.includes('write_inventory'))
  assert.ok(!result?.scopes.includes('read_inventory'))
  assert.ok(result?.scopes.includes('read_returns'))
  assert.ok(result?.scopes.includes('write_orders'))
  assert.ok(result?.scopes.includes('write_third_party_fulfillment_orders'))
  assert.match(result?.description || '', /write_inventory covers both/i)
  assert.match(result?.description || '', /blocks every provider write/i)
})
