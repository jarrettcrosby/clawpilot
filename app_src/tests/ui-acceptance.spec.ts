...
async function createTask(request: any, payload: Record<string, any>) {
  const res = await request.post('/api/tasks', { data: payload })
  expect(res.status()).toBe(201)
  return res.json()
}

async function getTask(request: any, id: string) {
  const res = await request.get('/api/tasks?includeArchived=true&includeQuarantined=true')
  expect(res.ok()).toBeTruthy()
  const arr = await res.json()
  return arr.find((t: any) => String(t.id) === String(id))
}
