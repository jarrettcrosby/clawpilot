...
const MAX_IMPORTED_ORDER_PAGES = 20
const CANONICAL_ORDER_PAGE_SIZE = 250
const MAX_CANONICAL_ORDER_PAGES = 20
...
function waitForOrderDiscoveryPoll(pollNumber: number) {
  const delay = ORDER_DISCOVERY_RUNNING_POLL_BACKOFF_MS[
    Math.min(
      pollNumber,
      ORDER_DISCOVERY_RUNNING_POLL_BACKOFF_MS.length - 1,
    )
  ]
...
type CanonicalOrderListPayload = {
  ok?: boolean
  error?: string
  code?: string
...
type OrderUnitWeightPayload = {
  ok?: boolean
  error?: string
  code?: string
  workspace?: OrderUnitWeightWorkspace
  result?: {
    workspace: OrderUnitWeightWorkspace | null
  }
}

function orderUnitWeightDraftValue(
  unitWeightGrams: number,
  system: MeasurementSystem,
) {
  return String(Number(gramsToDisplayWeight(
    unitWeightGrams,
    system,
...
export default function OperationsSection({
  ...
  const [planUnitWeightWorkspace, setPlanUnitWeightWorkspace] =
  ...
  const [planUnitWeightIdempotencyKey, setPlanUnitWeightIdempotencyKey] =
    useState('')
  const [savingPlanUnitWeights, setSavingPlanUnitWeights] = useState(false)
  ...
  const planPreparationAbortController = useRef<AbortController | null>(null)
  ...
  const loadWorkspace = useCallback(async (
    ...
    try {
      ...
      const importedOrders = [...operations.importedOrders]
      let importedOrderPage = operations.importedOrderPage
      if (importedOrderPage.returned !== importedOrders.length) {
        throw new Error('Imported-order pagination returned inconsistent evidence')
      }
      const seenCandidateGlobalIds = new Set(
        importedOrders.map((order) => order.candidateGlobalId),
      )
      const seenCursors = new Set<string>()
      let pagesLoaded = 1
      while (
        view === 'orders'
        && importedOrderPage.nextCursor
        && pagesLoaded < MAX_IMPORTED_ORDER_PAGES
      ) {
        const cursor = importedOrderPage.nextCursor
        if (seenCursors.has(cursor)) {
          throw new Error('Imported-order pagination did not advance')
        }
        ...
        const pagePayload = await pageResponse.json().catch(() => ({})) as
          ImportedOrderWorkbenchPayload
        if (
          !pageResponse.ok
          || !pagePayload.ok
          || !Array.isArray(pagePayload.orders)
          || !validImportedOrderPage(pagePayload.page)
        ) {
          throw new Error(
            pagePayload.error || 'The next imported-order page is unavailable',
          )
        }
        if (pagePayload.page.returned !== pagePayload.orders.length) {
          throw new Error('Imported-order pagination returned inconsistent evidence')
        }
        for (const order of pagePayload.orders) {
          if (seenCandidateGlobalIds.has(order.candidateGlobalId)) {
            throw new Error('Imported-order pagination returned a duplicate order')
          }
          seenCandidateGlobalIds.add(order.candidateGlobalId)
          importedOrders.push(order)
        }
        importedOrderPage = pagePayload.page
        pagesLoaded += 1
      }
      operations = {
        ...operations,
        ...
        importedOrderPage: {
          ...importedOrderPage,
          total: Math.max(importedOrderPage.total, importedOrders.length),
          returned: importedOrders.length,
          complete: importedOrderPage.nextCursor === null,
          truncated: importedOrderPage.nextCursor !== null,
        },
      }
      ...
      if (orderPage.returned !== canonicalOrders.length) {
        throw new Error('Order pagination returned inconsistent evidence')
      }
      const seenOrderGlobalIds = new Set(
        canonicalOrders.map((order) => order.globalId),
      )
      const seenOrderCursors = new Set<string>()
      let orderPagesLoaded = 1
      while (
        view === 'orders'
        && orderPage.nextCursor
        && orderPagesLoaded < MAX_CANONICAL_ORDER_PAGES
      ) {
        const cursor = orderPage.nextCursor
        if (seenOrderCursors.has(cursor)) {
          throw new Error('Order pagination did not advance')
        }
        seenOrderCursors.add(cursor)
        ...
        const orderPayload = await orderResponse.json().catch(() => ({})) as
          CanonicalOrderListPayload
        if (
          !orderResponse.ok
          || !orderPayload.ok
          || !Array.isArray(orderPayload.orders)
          || !validCanonicalOrderPage(orderPayload.page)
        ) {
          throw new Error(orderPayload.error || 'The next order page is unavailable')
        }
        if (orderPayload.page.returned !== orderPayload.orders.length) {
          throw new Error('Order pagination returned inconsistent evidence')
        }
        for (const order of orderPayload.orders) {
          if (seenOrderGlobalIds.has(order.globalId)) {
            throw new Error('Order pagination returned a duplicate order')
          }
          seenOrderGlobalIds.add(order.globalId)
          canonicalOrders.push(order)
        }
        orderPage = orderPayload.page
        orderPagesLoaded += 1
      }
      operations = {
        ...operations,
        ...
        orderPage: {
          ...orderPage,
          total: Math.max(orderPage.total, canonicalOrders.length),
          returned: canonicalOrders.length,
          complete: orderPage.nextCursor === null,
          truncated: orderPage.nextCursor !== null,
        },
      }
  ...
  const refreshOrders = useCallback(async () => {
    ...
    try {
        ...
        try {
          ...
          for (
            ...
            if (
              ...
              if (
                runningPolls
                >= ORDER_DISCOVERY_RUNNING_POLL_BACKOFF_MS.length
              ) {
                accountPollTimedOut = true
                break
              }
              await waitForOrderDiscoveryPoll(runningPolls)
              runningPolls += 1
              continue
            }
            runningPolls = 0
          ...
          if (accountComplete) {
            ...
            discoveryFailures.push(
              `${account.displayName} is still refreshing after the bounded wait. Current results are being reloaded and may not include that scan yet; refresh again after it finishes.`,
            )
          } else {
      ...
      const discoverySummary = discoveryTotals.accountsAttempted > 0
        ? `Checked ${discoveryTotals.accountsCompleted} of ${
            ...
            ? ` ${discoveryTotals.accountsInProgress} ${
              ...
              } still refreshing after the bounded wait.`
      ...
      const statusSummary = totals.attempted === 0
        ? 'No existing orders were ready for an exact status check.'
        : `Checked ${totals.attempted} existing sales-channel ${
  ...
  const loadPlanPreparation = async (
    ...
    try {
      ...
      const assignmentRequest = order.sourceProvider === 'shopify'
        ...
        : null
      const unitWeightParams = new URLSearchParams({
        accountGlobalId: order.planningPreparation.accountGlobalId,
        candidateGlobalId: order.planningPreparation.candidateGlobalId,
      ...
      const unitWeightRequest = fetch(
        `/api/operations/order-unit-weights?${unitWeightParams.toString()}`,
        { cache: 'no-store', signal },
      )
      const [response, assignmentResponse, unitWeightResponse] =
        await Promise.all([
          packagingRequest,
          assignmentRequest,
          unitWeightRequest,
      ...
      const unitWeightPayload = await unitWeightResponse.json()
        .catch(() => ({})) as OrderUnitWeightPayload
      if (signal?.aborted) return
      if (
        !unitWeightResponse.ok
        || !unitWeightPayload.ok
        || !unitWeightPayload.workspace
      ) {
        throw new Error(
          `${unitWeightPayload.error || 'Order unit weights could not be loaded'}${
            unitWeightPayload.code ? ` [${unitWeightPayload.code}]` : ''
          }`,
        )
      }
      ...
      setPlanUnitWeightWorkspace(unitWeightPayload.workspace)
      ...
      if (planPreparationAbortController.current?.signal === signal) {
  ...
  const openPlan = (trainingTarget?: ShadowTrainingPlanTarget) => {
    ...
    planPreparationAbortController.current = null
    ...
    setPlanUnitWeightIdempotencyKey(
      `operations-unit-weight:${detail?.globalId || 'order'}:${crypto.randomUUID()}`,
    )
    ...
    if (detail && !sealedTrainingEvidence) {
      const controller = new AbortController()
      planPreparationAbortController.current = controller
      void loadPlanPreparation(
        detail,
        Boolean(trainingTarget),
        controller.signal,
      )
    }
  }
  ...
  const closePlan = () => {
    if (planningOrder || creatingPlanEvidence || savingPlanUnitWeights) return
    planPreparationAbortController.current?.abort()
    planPreparationAbortController.current = null
    setPlanPreparationLoading(false)
    setPlanOpen(false)
  ...
  const savePlanUnitWeights = async () => {
    ...
    if (
      ...
      || !planUnitWeightIdempotencyKey
    ) return
    const lines = editableLines.flatMap((line) => {
      const displayWeight = Number(planUnitWeightDrafts[line.lineGlobalId])
      const unitWeightGrams = Number.isFinite(displayWeight)
        && displayWeight > 0
        ? displayWeightToGrams(
            displayWeight,
    ...
    setPlanError('')
    try {
      const response = await fetch('/api/operations/order-unit-weights', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Idempotency-Key': planUnitWeightIdempotencyKey,
        },
        body: JSON.stringify({
          accountGlobalId: preparation.accountGlobalId,
          candidateGlobalId: preparation.candidateGlobalId,
          candidateRowVersion: preparation.candidateRowVersion,
          lines,
          reason: planUnitWeightReason.trim(),
        }),
      })
      const payload = await response.json().catch(() => ({})) as
  ...
  const planUnitWeightWorkspaceRequired = (
    !shadowTrainingPlanTarget
    || planPackagingWorkspace !== null
  )
  const planUnitWeightWorkspaceBlocked = (
    planUnitWeightWorkspaceRequired
    && (
      planUnitWeightWorkspace === null
      || planUnitWeightHasUnsavedChanges
      || planUnitWeightWorkspace.missingLines.length > 0
    )
  )
  useEffect(() => {
    if (
    ...
    if (entries.length) {
      setPlanCartonizationEvidenceGlobalId('')
      setPlanEvidenceIdempotencyKey(
        `operations-rate-plan:${selectedGlobalId || 'order'}:${crypto.randomUUID()}`,
      )
    }
  }, [
    measurementSystem,
                        ...
                        {planUnitWeightEditingAllowed ? (
                          <>
                            <TextField
                              ...
                              label="Audit reason"
                              value={planUnitWeightReason}
                              disabled={savingPlanUnitWeights}
                              onChange={(event) => {
                                setPlanUnitWeightReason(event.target.value)
                                setPlanError('')
                              }}
                              inputProps={{ maxLength: 500 }}
                            />
                            <Button
                              ...
                              disabled={
                                savingPlanUnitWeights
                                || planUnitWeightHasInvalidDraft
                                || !planUnitWeightHasUnsavedChanges
                                || planUnitWeightReason.trim().length < 8
                              }
                              startIcon={savingPlanUnitWeights
                                ? <CircularProgress size={16} />
                                : undefined}
