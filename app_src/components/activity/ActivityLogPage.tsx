...
export default function ActivityLogPage({ tasks, pipelineEntries = [], defaultModule, onClose }: Props) {
  ...
  const effectiveFilterModule = userChangedModule ? filterModule : (defaultModule || 'all')
  ...
  const filtered = useMemo(() => allEntries.filter(e => {
    if (effectiveFilterModule !== 'all' && e._module !== effectiveFilterModule) return false
    ...
    return true
  }), [allEntries, effectiveFilterModule, filterType, filterActor, readFilter, readIds, search])
    ...
    <Box sx={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      <Box sx={{ px:3, pt:2.5, pb:2, borderBottom:'1px solid rgba(255,255,255,0.06)', flexShrink:0 }}>
        <Stack direction="row" alignItems="flex-start" justifyContent="space-between" mb={1.5}>
          <Box>
            ...
            <Typography variant="caption" color="text.disabled">
              {MODULE_LABELS[effectiveFilterModule] || effectiveFilterModule} ·{' '}
        ...
        <Stack direction="row" spacing={0.75} mb={1.5} sx={{ overflowX:'auto', pb:0.5 }}>
          {['all','projects','docs','pipeline','agents','dashboard'].map(m => (
            <Chip key={m} size="small" label={MODULE_LABELS[m]||m} onClick={() => { setFilterModule(m); setUserChangedModule(true) }}
              sx={{ height:26, fontSize:'0.72rem', borderRadius:2, cursor:'pointer', flexShrink:0,
                backgroundColor: effectiveFilterModule===m ? (userChangedModule && m !== 'all' ? 'rgba(168,199,250,0.2)' : 'rgba(168,199,250,0.15)') : 'rgba(255,255,255,0.05)',
                color: effectiveFilterModule===m ? '#A8C7FA' : 'text.disabled',
                border: effectiveFilterModule===m ? `1px solid ${userChangedModule && m !== 'all' ? 'rgba(168,199,250,0.6)' : 'rgba(168,199,250,0.3)'}` : '1px solid transparent',
                fontWeight: effectiveFilterModule===m && userChangedModule && m !== 'all' ? 700 : 400,
