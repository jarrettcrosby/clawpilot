...
export default function CardDetailDrawer({ task, open, onClose, onUpdate, onArchive, actionabilityHint }: Props) {
  ...
  useEffect(() => {
    setNextActionVal(task?.workItem?.nextAction || '')
  }, [task?.id, task?.workItem?.nextAction])
              ...
              <Stack direction="row" spacing={1} alignItems="center" sx={{ flex: 1 }}>
                <TextField
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault()
                      patch({ nextAction: nextActionVal.trim() })
                    }
                  }}
                  ...
                  sx={{
                    ...
                    '& .MuiOutlinedInput-root': {
                      ...
                      borderRadius: 2,
                      fontSize: '0.78rem',
                      ...(highlightNextAction ? { boxShadow: '0 0 0 2px rgba(255,167,38,0.2)' } : {}),
                    },
                    ...(highlightNextAction ? { '& .MuiOutlinedInput-notchedOutline': { borderColor: '#FFA726' } } : {}),
                  }}
                />
                <Button
                  size="small"
                  variant="outlined"
                  onClick={() => patch({ nextAction: nextActionVal.trim() })}
