...
function statusTone(status: string): ChipTone {
  ...
  if (/pending|partial|open|review|mapping|stale|waiting|processing/i.test(status)) return 'warning'
  return 'default'
}
...
function draftCount(value: unknown, keys: string[]) {
  if (Array.isArray(value)) {
    const statuses = new Set(keys.map((key) => key.replace(/[A-Z]/g, (character) => `_${character.toLowerCase()}`)))
    return recordList(value).filter((item) => statuses.has(textValue(item, ['status']).toLowerCase())).length
  }
  const source = record(value)
  const counts = record(firstValue(source, ['counts', 'summary']))
  return Math.max(0, Math.round(numberValue(Object.keys(counts).length ? counts : source, keys)))
}
...
export default function PosSection() {
  ...
  const draftMetrics = [
    ...
    { label: 'Approved', value: draftCount(drafts, ['approved']), color: '#CFC6EA' },
    { label: 'Posted', value: draftCount(drafts, ['posted']), color: '#70D6A7' },
    { label: 'Failed', value: draftCount(drafts, ['failed']), color: '#FF8A80' },
  ]
  const reconciledDrafts = accountingDrafts.filter((draft) => textValue(draft, ['reconciliationStatus']) === 'ready').length
          ...
          {capabilities?.canView === false ? (
                    ...
                    <TableBody>
                      {filteredOrders.map((order, index) => {
                        ...
                        return (
                          ...
                          >
                            <TableCell><Typography variant="body2" fontWeight={650}>{orderLabel(order)}</Typography><Typography variant="caption" color="text.disabled">Toast order</Typography></TableCell>
                            <TableCell>{dateLabel(firstValue(order, ['businessDate', 'date']))}</TableCell>
                    ...
                    return (
                      ...
                      >
                        <Box minWidth={0}>
                          ...
                          <Typography variant="caption" color="text.disabled" display="block" noWrap>Toast order detail</Typography>
                        </Box>
            ...
            <Stack spacing={2}>
              <Box display="flex" justifyContent="space-between" alignItems="flex-start" gap={2}>
                <Box>
                  <Typography fontWeight={700}>Accounting draft queue</Typography>
                  <Typography variant="caption" color="text.secondary">Toast sales reconciliation</Typography>
                </Box>
              ...
              <Box display="grid" gridTemplateColumns={{ xs: '1fr', lg: 'minmax(0, 1fr) minmax(280px, 0.42fr)' }} gap={2}>
                <Box sx={{ ...panelSx, overflow: 'hidden' }}>
                  <Box px={2} py={1.5} borderBottom="1px solid rgba(255,255,255,0.08)">
                    <Typography fontWeight={700}>Drafts in range</Typography>
                  </Box>
                  {accountingDrafts.length ? accountingDrafts.map((draft, index) => {
                    const status = displayStatus(textValue(draft, ['status'], 'Pending'))
                    ...
                    return (
                      <Box key={textValue(draft, ['id', 'guid'], String(index))} sx={{ px: 2, py: 1.25, display: 'grid', gridTemplateColumns: { xs: 'minmax(0, 1fr) auto', sm: 'minmax(110px, 0.55fr) minmax(0, 1fr) auto auto auto' }, alignItems: 'center', gap: 1.25, borderBottom: '1px solid rgba(255,255,255,0.065)' }}>
                        <Box minWidth={0}>
                        ...
                        <Chip size="small" variant="outlined" color={statusTone(status)} label={status} />
                        <Typography variant="caption" color="text.secondary" noWrap sx={{ gridColumn: { xs: '1 / -1', sm: 'auto' } }}>
                    ...
                    <Box minHeight={150} display="grid" sx={{ placeItems: 'center' }} px={2}>
                      <Typography variant="body2" color="text.secondary">No accounting drafts in this range</Typography>
                    </Box>
                  )}
                </Box>
