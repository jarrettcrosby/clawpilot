...
function columns(entity: CrmEntity) {
  if (entity === 'organizations') return [
    ['name', 'Organization'], ['accountType', 'Type'], ['accountManager', 'Owner'], ['phone', 'Phone'],
  ] as const
            ...
            <TableBody>
              {records.map((record) => (
                <TableRow
                  ...
                  hover={editable}
                  onClick={() => { if (editable) void openEditor(record) }}
              ...
              {records.length === 0 && (
                <TableRow><TableCell colSpan={5} align="center" sx={{ py: 6, color: 'text.secondary' }}>No records</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        )}
