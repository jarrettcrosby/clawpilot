...
async function queryEntityPage(
  ...
  const query = encodeURIComponent(
    `SELECT * FROM ${entity} STARTPOSITION ${startPosition} MAXRESULTS ${QUERY_PAGE_SIZE}`,
  )
  return request(
...
function sourceRowCount(payload: Record<string, unknown>, entity: 'Account' | 'Item') {
...
async function queryAllEntities<T>(
  ...
  for (let startPosition = 1; startPosition <= MAX_CATALOG_RECORDS; startPosition += QUERY_PAGE_SIZE) {
    const payload = await queryEntityPage(entity, ownerEmail, connectionId, startPosition)
    for (const record of parse(payload)) {
      const id = String((record as { id?: unknown }).id || '')
      if (id) records.set(id, record)
    }
    if (sourceRowCount(payload, entity) < QUERY_PAGE_SIZE) return [...records.values()]
  }
...
export async function readQuickBooksCatalog(ownerEmail: string, connectionId: string) {
  const [company, accounts, items] = await Promise.all([
