...
export const SHOPIFY_CAPABILITY_SCOPES = {
  ...
  // The shipped control plane intentionally bounds initial history to a fixed
  // rolling 60-day window. read_all_orders is observed as additional grant
  // evidence but is not required for this bounded mode.
  historical_order_import: ['read_orders'],
...
export const COMMERCE_CUSTOM_INTEGRATION_ONBOARDING = {
  shopify: {
    ...
    requiredBeforeConnect: [
      ...
      'Copy a store-owned myshopify.com domain, client ID, and client secret. ClawPilot verifies and stores Shopify\'s permanent canonical domain.',
    ],
    ...
    webhookSetupGroups: [
      ...
      },
      {
        ...
        state: 'available',
        behavior: 'Seven minimized core-order signals trigger exact read-only order refresh; scheduled polling remains the backstop. Delete, refund, and return-specific signals require separate tombstone or resource-resolution processors before this can be called exhaustive.',
      },
      {
...
export const CLAWPILOT_SHOPIFY_CAPABILITY_IMPLEMENTATION = {
  ...
  historical_order_import: 'control_plane_implemented',
