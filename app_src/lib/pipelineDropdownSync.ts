...
async function getSheetIdByTitle(title: string): Promise<number> {
  const meta = await sheetGet(`/google-sheets/v4/spreadsheets/${SHEET_ID}?fields=sheets.properties`)
...
export async function pullDropdownsFromSheet() {
  ...
  try {
    ...
    const data = await sheetGet(`/google-sheets/v4/spreadsheets/${SHEET_ID}/values/${encoded}`)
...
export async function pushDropdownsToSheet(input: DropdownCatalog) {
  ...
  try {
    ...
    // value update
    await matonFetch(`/google-sheets/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(clearRangeA1)}:clear`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
    })

    await matonFetch(`/google-sheets/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(rangeA1)}?valueInputOption=USER_ENTERED`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ range: rangeA1, majorDimension: 'ROWS', values: rows }),
    })
