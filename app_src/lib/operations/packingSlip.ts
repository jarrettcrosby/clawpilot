...
export type PackingSlipSnapshot = {
  ...
  shipmentSectionTitle?: string
  shipmentTimestampLabel?: string
  footerNotice?: string
...
export function renderPackingSlip(snapshot: PackingSlipSnapshot): RenderedPackingSlip {
  ...
  const pages = Array.from({ length: pageCount }, (_, pageIndex) => {
    ...
    text(snapshot.documentTitle || 'ClawPilot Packing Slip', 48, 744, 18, true)
    ...
    text(snapshot.shipmentSectionTitle || 'Shipment', 330, 682, 10, true)
    text(`${snapshot.carrier} ${snapshot.serviceCode}`, 330, 664, 10)
    text(snapshot.trackingNumber, 330, 650, 9)
    text(
      `${snapshot.shipmentTimestampLabel || 'Confirmed'} ${snapshot.shippedAt}`,
      330,
      636,
      9,
    )
