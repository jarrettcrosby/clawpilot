...
function applicationUserGuideContent(user: ApplicationGuideUser): string {
  ...
  return [
    ...
    '2. Imported orders must resolve to an existing CRM customer and active CRM product before inventory can be promised or reserved.',
    '3. Review warehouse feasibility, promise date, carton plan, selected carrier rate, expected cost, expected revenue, and margin before releasing live work.',
    '4. Warehouse execution progresses through release, wave, pick, pack, label, print, shipment, and source-channel update. Every durable change emits an auditable event.',
    '5. Inventory changes are append-only ledger entries. Do not correct inventory by editing totals; use a controlled adjustment with a reason and source reference.',
    '6. Contract directives create immutable billable events. Reconciliation and invoicing consume those events without changing their original amount or evidence.',
    '7. The current proof command uses mock commerce, carrier, and printer adapters. Mock shipments are operational acceptance evidence, not real customer fulfillment.',
    '',
