    ...
    await client.query(
      `
        ...
        SET status = CASE WHEN attempts >= $1 THEN 'dead' ELSE 'failed' END,
            ...
            lock_token = NULL,
            updated_at = now()
        WHERE target_system IN ('google_sheets', 'google_workspace', 'google_workspace_v2', 'pipeline_internal_v1')
    ...
    query<CountByStatusRow>(
      `
        ...
        WHERE aggregate_type LIKE 'pipeline%'
          AND target_system IN ('google_sheets', 'google_workspace', 'google_workspace_v2', 'pipeline_internal_v1')
          AND (
            (
