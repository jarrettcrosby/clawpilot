...
export function getTaskState(task: Task): { state: TaskState; missing: string[]; reasons: string[] } {
  ...
  if ((task as any)?.deletedAt) return { state: 'deleted', missing, reasons }
  ...
  if (!task.assignee) missing.push('owner')
...
export function getAutoPickupEligibility(task: Task): AutoPickupEligibility {
  ...
  if (!task.assignee) reasons.push('no-assigned-agent')
  if ((task.tags || []).includes('blocked')) reasons.push('blocked-tag')
