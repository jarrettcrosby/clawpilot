...
function isPublicApi(pathname: string) {
  return pathname === '/api/health' || pathname === '/api/client-error' || pathname.startsWith('/api/auth/')
}
...
export function middleware(req: NextRequest) {
  ...
  if (missing.length > 0) {
    const message = `Dev runtime requires isolated data env vars. Missing: ${missing.join(', ')}. Start via scripts/dev-start.sh from /Users/agentsuburbiasandwich/Desktop/clawd-app-dev.`
