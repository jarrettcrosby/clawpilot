...
import { isPostgresExecutionStoreEnabled, summarizeExecutionRunsFromPostgres } from '@/lib/persistence/execution'
...
export async function GET(req: NextRequest) {
  if (isPostgresExecutionStoreEnabled()) {
    try {
      ...
      return NextResponse.json(await summarizeExecutionRunsFromPostgres({ operatorId: actor.email, boardId: board.id }))
    } catch (error) {
      if (!shouldFallbackToFileOnDatabaseError()) throw error
      console.warn('[execution-runs-summary] Postgres read failed; falling back to file store', error)
    }
  }

  const { count, last } = await summarizeJsonl(EXECUTION_RUNS_FILE)
