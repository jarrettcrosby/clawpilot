...
export async function GET(
  ...
  try {
    ...
    if (!isPostgresStorageEnabled()) {
      return json({
        ok: false,
        error: 'POS catalog item details require Postgres storage',
    ...
    return json({
      ...
      item: {
        ...
        imageUrl: item.hasImage
          ? `/api/pos/catalog/items/${item.restaurantGuid}/${item.itemGuid}/image`
          : null,
    ...
    console.error('[pos catalog item detail] request failed')
    return json({
      ok: false,
      error: 'Toast product details are temporarily unavailable',
      code: 'POS_CATALOG_ITEM_INTERNAL_ERROR',
    }, 500)
  }
}
