 ...
 * Email-only result presentation; non-Email rendering follows SuiteCRM 8.10.1.
...
{else}
    ...
    {if $module === 'Emails'}
    <table class="list view clawpilot-email-search">
        <thead>
            <tr><th>Subject</th><th>From</th><th>Global ID</th><th>Date Sent / Received</th><th>Date Created</th></tr>
        </thead>
        <tbody>
            {foreach from=$clawpilotEmailRows item=email}
            <tr class="{cycle values="oddListRowS1,evenListRowS1"}">
                <td><a target="_top" href="{$SITE_URL|escape:'html'}/{$email.detailPath|escape:'html'}">{$email.subject|escape:'html'}</a></td>
                <td>{$email.from|default:'Unavailable'|escape:'html'}</td>
                <td>{$email.globalId|default:'—'|escape:'html'}</td>
                <td>{$email.sentAt|default:'—'|escape:'html'}</td>
                <td>{$email.createdAt|default:'—'|escape:'html'}</td>
            </tr>
            {/foreach}
        </tbody>
    </table>
