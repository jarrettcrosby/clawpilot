...
try {
  ...
  let failVerification = false, trashed = false
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/auth/login-email') {
      if (request.method() === 'GET') return json({ ok: true, loginEmail: owner.email })
  ...
  console.log('PASS: real React login verification/retry/navigation, organization Trash/Restore, empty states and mobile width')
} finally {
  await browser?.close()
