...
requireAll(setupPanel, [
  ...
  'All eligible customers may prepare saved-address rates after signing in.',
  'Product, inventory, packaging, and ${selectedRateEnvironment',
...
requireAll(setupPanel, [
  ...
  'Only mapped, in-stock items with usable packaging, destination, origin, and carrier service receive rates.',
  'This does not change checkout access or customer eligibility.',
  "label: 'Shopify account changes'",
  "value: 'None'",
], 'ordered checkout-audience prerequisite')
for (const fragment of [
  'These desired controls are independent of Advanced safety',
  'Prove a live cart request',
  'zero callback provider writes',
  ...
  assert.equal(
    setupPanel.includes(fragment),
    false,
    `checkout setup must not expose advisory copy: ${fragment}`,
  )
}
requireAll(setupPanel, [
  ...
  'Checkout',
  'access and rate source apply to this account.',
