...
assert.ok(railwayStart.includes('application did not pass readiness validation'))
...
assert.ok(railwayStart.includes('READINESS_URL="${HEALTH_URL}?probe=readiness"'))
assert.ok(railwayStart.includes('response.ok ? 0 : 1'))
assert.ok(railwayStart.includes('AbortSignal.timeout(10000)'))
assert.ok(railwayStart.includes('for _attempt in $(seq 1 25)'))
assert.ok(
  livenessProbePosition >= 0
    && workerStartPosition > livenessProbePosition
    && readinessProbePosition > workerStartPosition,
  'Railway startup must use liveness before workers and bounded readiness after workers',
)
for (const fragment of [
...
for (const fragment of [
  ...
  assert.ok(healthRoute.includes(fragment), `Health readiness contract missing ${fragment}`)
}
assert.equal(
  JSON.parse(read('railway.json'))?.deploy?.healthcheckPath,
