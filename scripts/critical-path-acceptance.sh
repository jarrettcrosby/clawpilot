...
echo "4) Assign agent to target task"
HTTP=$(curl -sS -o /tmp/_cp_assign.json -w "%{http_code}" -X PATCH "$BASE/api/tasks" \
  -H 'Content-Type: application/json' \
  -d "{\"id\":\"$TARGET_TASK_ID\",\"assignedAgent\":\"$AGENT_ID\",\"_actor\":\"$ACTOR\"}")
...
# action panel sanity: at least one actionable task exists and payload parses
jq -e 'map(select(.status == "todo" or .status == "in-progress" or .status == "review")) | length >= 1' /tmp/_cp_tasks_now.json >/dev/null || fail "no actionable tasks in dashboard data"
