...
assertIncludes(pipelineProvisioning, 'canonicalConfiguredDropdownRows(currentDropdownResult.values || [])', 'configured dropdown canonicalization')
...
assertIncludes(pipelineProvisioning, 'export async function rebuildPipelineTabsWithRequest', 'legacy managed-tab workbook rebuild')
assertIncludes(read('app_src/lib/pipelineLegacyWorkbook.ts'), 'rebuildPipelineTabsWithRequest', 'legacy managed-tab reset adapter')
...
assertIncludes(pipelineProvisioning, "Interactions: ['Priority', 'Type', 'Owner', 'Organization'", 'interaction workbook type and organization headers')
...
assertIncludes(suiteCrmGlobalIdBootstrap, "ensure_global_id_field('Emails', true, false)", 'native SuiteCRM Email Global ID search field')
...
assertIncludes(suiteCrmGlobalIdBootstrap, "[CLAWPILOT_PRODUCT_MODULE, 'Emails']", 'SuiteCRM Email global search enablement')
...
assertIncludes(suiteCrmGlobalIdBootstrap, "enable_and_verify_global_search_modules([CLAWPILOT_PRODUCT_MODULE, 'Emails'])", 'SuiteCRM boot-time Product and Email search contract')
