...
assert.ok(!demoMode.includes('DEMO_PIPELINE_ID'))
...
assert.ok(workspaceMemberships.includes('reference_access_disabled = false'))
assert.ok(workspaceMemberships.includes('const activePipelineId = activePipeline.rows[0]?.id'))
assert.ok(workspaceMemberships.includes('pipeline.id <> $3::uuid'))
assert.ok(!workspaceMemberships.includes('DEMO_PIPELINE_ID'))
...
assert.ok(seed.includes('PIPELINE_ID = crypto.randomUUID()'))
assert.ok((seed.match(/crypto\.randomUUID\(\)/g) || []).length >= 7)
assert.ok(!seed.includes('function uuid(prefix, index)'))
assert.ok(seed.includes('DEMO_QUARANTINE_ROTATION_ENABLED'))
assert.ok(seed.includes("mode: 'quarantine_guard_rollout'"))
assert.ok(seed.includes('pendingImmutablePipelines'))
assert.ok(seed.includes('immutableDemoPipelines'))
assert.ok(seed.includes('immutablePipelineIds'))
assert.ok(seed.includes('deletablePipelineIds'))
...
assert.ok(seed.includes('reference_access_disabled = true'))
assert.ok(seed.includes('reference_access_disabled,'))
assert.ok(seed.includes("outbox.status IN ('queued', 'failed', 'processing')"))
assert.ok(seed.includes('active SuiteCRM work item(s)'))
assert.ok(seed.includes("SET status = 'dead'"))
assert.ok(seed.includes("available_at = 'infinity'::timestamptz"))
assert.ok(seed.includes("payload->>'pipelineId' = ANY($1::text[])"))
assert.ok(seed.includes('WITH legacy_references AS'))
assert.ok(seed.includes('expanded_legacy_references'))
assert.ok(seed.includes('SELECT survivor_reference_code FROM crm_contact_merges'))
assert.ok(seed.includes('SELECT alias.alias_code'))
assert.ok(seed.includes("link.slug = 'mail-' || legacy.reference_code"))
assert.ok(seed.includes('legacy.reference_code = ANY(link.tags)'))
assert.ok(seed.includes('UPDATE short_links link'))
assert.ok(seed.includes('disabled_at = COALESCE(link.disabled_at, now())'))
assert.ok(
  seed.indexOf('FROM workspace_organizations') < seed.indexOf('const currentDemoPipelines'),
  'demo seed must lock workspace before pipelines to match demo-entry lock order',
)
...
assert.ok(verifier.includes('accessible_legacy_pipelines'))
assert.ok(verifier.includes('enabled_legacy_crm_short_links'))
assert.ok(verifier.includes('expanded_legacy_references'))
assert.ok(verifier.includes('legacy_pipeline_memberships'))
assert.ok(verifier.includes('legacy_default_preferences'))
assert.ok(verifier.includes('active_legacy_outbox'))
assert.ok(verifier.includes('missing_legacy_outbox_evidence'))
assert.ok(verifier.includes('nonterminal_legacy_outbox_evidence'))
...
assert.ok(postgresAcceptance.includes("DEMO_QUARANTINE_ROTATION_ENABLED: 'true'"))
assert.ok(postgresAcceptance.includes('runExpectingFailure'))
assert.ok(postgresAcceptance.includes('verifyQuarantine'))
assert.ok(postgresAcceptance.includes('staleCutoff'))
