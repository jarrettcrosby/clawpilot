...
validate_invariants() {
  local health_code runtime_code root_code runtime_json lane repo port
  ...
  INVARIANT_SNAPSHOT=$(python3 - <<'PY' "$health_code" "$runtime_code" "$lane" "$repo" "$port" "$root_code"
import json, sys
health, runtime, lane, repo, port, root = sys.argv[1:]
print(json.dumps({
  "healthCode": health,
  "runtimeCode": runtime,
  "runtimeLane": lane,
