...
async function runPostgresNotificationAcceptance() {
  ...
  const actorEmail = 'accounting-owner@example.test'
  let pool
  try {
    command('docker', [
    ...
    const scope = { organizationId, restaurantGuid, businessDate: '2026-07-18' }
    ...
    assert.deepEqual(deliveries.rows, [{ occurrence: 1, status: 'pending' }])
