...
requireAll(panel, [
  'Checkout audience',
  'Restricted default · no matching policy means no ClawPilot rates.',
  'Desired customer policies remain editable in every Advanced safety',
  'currently pauses the effective callback with an empty response',
  'saved policy intent remains intact.',
  'Shopify does not guarantee that',
  'authenticated Customer GID',
  'The effective callback still fails closed',
  'no customer-count cap',
  "effectiveActivation !== 'missing'",
...
requireAll(panel, [
  ...
  'TEST cannot create an effective proof lane on a production Shopify store',
  'no timed proof is started, and the $0 proof subsidy is unavailable',
  ...
  '15–240 minute proof window or Until turned off',
  'caches successful rates without customer identity',
  'Desired LIVE policies are saved locally with zero Shopify writes',
  'production store returns authenticated empty rates',
  'Shopify writes',
...
requireAll(panel, [
  ...
  'TEST proof expired · fails closed',
  'policy.shadowExpiresAt',
  "policy.shadowLifetimeMode === 'until_turned_off'",
  'TEST proof · Until turned off',
], 'explicit TEST lifetime controls')
...
], 'tenant-configured TEST subsidy controls')
