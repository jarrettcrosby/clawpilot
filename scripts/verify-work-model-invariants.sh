...
def is_testing_record(task):
    hay = ' '.join([
        str(task.get('title') or ''),
        str(task.get('desc') or ''),
        str(task.get('description') or ''),
        str(task.get('category') or ''),
        *[str(x or '') for x in (task.get('labels') if isinstance(task.get('labels'), list) else [])],
        *[str(x or '') for x in (task.get('tags') if isinstance(task.get('tags'), list) else [])],
    ]).lower()
    markers = [
        'test', 'testing', 'synthetic', 'dummy', 'placeholder', 'sandbox',
        'qa', 'doer mode', 'ui acceptance governance', 'critical path acceptance',
        'aig legit flow',
    ]
    return any(marker in hay for marker in markers)

def assignment_projection_eligible(task):
    return bool(task.get('assignedAgent')) and not task.get('archived') and not task.get('deletedAt') and not is_testing_record(task)
...
if os.path.exists(assignments_path):
    ...
    if isinstance(assignments, list):
        ...
        for t in tasks:
            if not assignment_projection_eligible(t):
                continue
            tid = str(t.get('id',''))
            assigned = t.get('assignedAgent')
            a = by_task.get(tid)
