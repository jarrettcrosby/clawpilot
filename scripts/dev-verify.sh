...
readarray -t RUNTIME_FIELDS < <(python3 - <<'PY' "$RUNTIME_JSON"
import json,sys
try:
...
RUNTIME_COMMIT="${RUNTIME_FIELDS[0]:-}"
RUNTIME_LANE="${RUNTIME_FIELDS[1]:-}"
RUNTIME_PORT="${RUNTIME_FIELDS[2]:-}"
...
if [[ -n "${ROUTE_CHECKS:-}" ]]; then
  ...
  echo "INFO: no ROUTE_CHECKS provided"
fi
