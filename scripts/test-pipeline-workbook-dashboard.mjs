...
const crmPersistence = readFileSync(resolve(process.cwd(), 'app_src/lib/persistence/crm.ts'), 'utf8')
...
assert.match(projectionSource, /await configurePipelineTabs\(runtime, input\.context\.sheetId, dataRowCounts\)[\s\S]{0,260}await applyPipelineWorkbookBranding/)
assert.match(projectionSource, /const branding = await readPipelineWorkbookBranding\(input\.context\.pipelineId\)/)
assert.match(projectionSource, /await configureLegacyPipelineTabs\(input\.context\.sheetId, dataRowCounts\)[\s\S]{0,180}await applyLegacyPipelineWorkbookBranding\(input\.context\.sheetId, branding\)/)
assert.match(projectionSource, /readCrmWorkbookProjectionSnapshotInPostgres/)
assert.ok(
  projectionSource.indexOf('const projection = await readCrmWorkbookProjectionSnapshotInPostgres')
    < projectionSource.indexOf('const binding = await resolvePipelineSheetBindingInPostgres'),
  'source count and capacity must be validated before any Google workbook binding or write',
)
assert.doesNotMatch(projectionSource, /limit: 1000/)
assert.match(configureBlock, /Math\.max\(1000, projectedDataRows \+ 4\)/)
assert.match(crmPersistence, /CRM_WORKBOOK_PROJECTION_DATA_ROW_CAPACITY = 19_996/)
assert.match(crmPersistence, /SET TRANSACTION ISOLATION LEVEL REPEATABLE READ READ ONLY/)
assert.match(crmPersistence, /AND \(\$2::uuid IS NULL OR opportunity\.id > \$2::uuid\)/)
assert.match(crmPersistence, /source-count mismatch/)
assert.match(crmPersistence, /assertCrmWorkbookProjectionCountsWithinCapacity\(counts\)/)
