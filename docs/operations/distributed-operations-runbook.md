...
7. If the heartbeat is stale, verify the Railway poller process, `SHOPIFY_INVENTORY_REFRESH_POLL_MS`, worker secret, commerce feature gate, and migrations `0169`, `0171`, and `0172` before restarting. If leases are stale or jobs are overdue/dead, preserve the job and provider-attempt evidence; do not delete rows or bypass the queue. Migration `0172` lets the worker rotate an expired lease only when the immutable capture matches the exact organization, account, attempt, and request hash; recovery reprojects that capture without another Shopify read. Disable checkout quotes for the affected account until a current successful inventory run restores health.
