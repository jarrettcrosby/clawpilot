...
Use this procedure only for the trusted Railway development environment. It leaves Express Parcel International DBA EPISCS as the UPS/FedEx credential owner and creates AG Alchemy, LLC projections for sandbox rating only.
...
8. Stop after rating for every connection that lacks `sandbox_label`, including the AG delegated accounts. Only a separately authorized diagnostic-label connection may choose one exact returned service and use the Settings procedure below or the order-bound procedure for an eligible packed order. Pickup, manifest, tracking mutation, and production actions remain prohibited.

## Settings Sandbox Rate, Label, Print, And Void Diagnostic

This procedure does not apply to AG's EPISCS-managed rating-only connections. Their successful rate is terminal because they do not authorize `sandbox_label`.
...
4. Confirm the test-label Global ID, service, format, byte length, checksum, tracking reference, and `created` state. The current thermal diagnostic requests UPS ZPL or FedEx `ZPLII` with `STOCK_4X6`; ClawPilot normalizes the FedEx format name to `ZPL`, validates the decoded UTF-8 command envelope, and persists the returned bytes unchanged. Existing PDF/PNG artifacts remain immutable and require a printer or print service that advertises that exact format and media. Never convert, scale, or inject margin commands into carrier label bytes. The checksum and length cover the actual bytes that will be delivered.
