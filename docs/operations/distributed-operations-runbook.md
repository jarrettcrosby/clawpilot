...
1. Confirm the active organization and open **Settings > Integrations > Sales channels**. The Shopify store or Faire brand must be configured, verified, and product-readable. The verified connection is the authorization for automatic read-only product catalog sync; do not ask the customer for a second ClawPilot approval, page approval, or product approval. This workflow is available only when `CLAWPILOT_COMMERCE_INTAKE_ENABLED=1` in development, local, or preview; production is refused. Only an organization owner or user with **Manage operations** may use the interactive controls.
