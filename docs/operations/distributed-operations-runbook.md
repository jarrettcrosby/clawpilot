...
container, exercise the `0178` fences and tombstones, and remove the container
afterward. A passing run is database acceptance evidence only; it is not
evidence of Shopify provider enforcement.
...
### AG Alchemy Development Carrier Sandbox Delegation

Use this procedure only for the trusted Railway development environment. It leaves Express Parcel International DBA EPISCS as the UPS/FedEx credential owner and creates AG Alchemy, LLC projections that default to sandbox rating only.

1. Confirm EPISCS has exactly one active, verified UPS sandbox account and exactly one active, verified FedEx sandbox account. Confirm AG has exactly one active owner or administrator and exactly one active warehouse with code `AG-ALCHEMY-01` and Global ID `gwh5366613`. Do not copy a client ID, secret, access token, or account number into the shell.
2. From the repository root, load only the linked ClawPilot development environment and run the read-only default plan:

   ```sh
   railway run --service clawpilot --environment development -- node scripts/establish-ag-alchemy-carrier-sandbox.mjs
   ```

3. Review the plan. It must identify the compiled trusted development database fingerprint, EPISCS as the unchanged credential owner/source, AG warehouse `gwh5366613` as sender origin, fresh AG-scoped target identities, `operator_owned` ownership, and only `sandbox_rate` in `allowedCapabilities`. Stop if it proposes an unexpected database, source, target, warehouse, provider, environment, mutation, or provider write. The command refuses a mismatched database identity even when Railway project variables are present.
4. Apply only that unchanged plan with the literal confirmation:

   ```sh
   railway run --service clawpilot --environment development -- node scripts/establish-ag-alchemy-carrier-sandbox.mjs --apply --confirm=establish-ag-alchemy-carrier-sandbox-v1
   ```

5. Confirm the result reports zero provider writes and zero labels, fresh successful UPS and FedEx credential verification, and create, update, or no-op target dispositions. Re-run the read-only plan; an unchanged establishment must report no-op dispositions. The source-state digest and EPISCS integration/account/ciphertext metadata must be unchanged.
6. Switch to AG Alchemy and open **Settings > Integrations > Shipping**. Confirm both connections are labeled **EPISCS-managed sandbox rating**, the sender origin is AG warehouse `gwh5366613`, and the only executable diagnostic is **Test sandbox rate**. The generic **Test connection** action is intentionally unavailable because it could mutate verification state; the server also rejects it before provider I/O. Credential reveal, credential/account edits, disconnect, label, void, pickup, manifest, shipment, and every provider-mutation action must remain unavailable.
7. To enable the bounded sandbox label diagnostic, first run another plan with `--enable-sandbox-fulfillment`. Confirm the proposed profile is exactly `sandbox_fulfillment_diagnostic`, the ordered capabilities are exactly `sandbox_rate` and `sandbox_label`, and every source, target, warehouse, credential-owner, and zero-provider-write fact remains unchanged. Apply only with its separate literal confirmation:

   ```sh
   railway run --service clawpilot --environment development -- node scripts/establish-ag-alchemy-carrier-sandbox.mjs --enable-sandbox-fulfillment
   railway run --service clawpilot --environment development -- node scripts/establish-ag-alchemy-carrier-sandbox.mjs --enable-sandbox-fulfillment --apply --confirm=enable-ag-alchemy-carrier-sandbox-fulfillment-diagnostics-v1
   ```

   Switch back to AG Alchemy. The Shipping panel must say **EPISCS-managed sandbox fulfillment diagnostics** and expose the Rate, Create label, Print stored label, and Void / close steps. Credentials and the full billing account number must remain concealed. Production labels, shipments, pickups, manifests, tracking mutations, and commerce writes must remain unavailable. To revoke this diagnostic capability, rerun steps 2 through 4 without `--enable-sandbox-fulfillment`; the rating-only profile must immediately hide and server-reject label creation, stored PDF/PNG/ZPL download, print, void, and close.
8. Run the proof harness in plan mode first, then execute it only with its literal confirmation. It accepts either exact managed profile, authenticates to the hosted development app, selects AG Alchemy, validates that the current carrier sender name and registered address exactly match warehouse `gwh5366613`, requests one UPS and one FedEx sandbox quote, and asserts exactly two new `grq` success records with no label-attempt, label, shipment, print-artifact, or print-job change:

   ```sh
   railway run --service clawpilot --environment development -- node scripts/prove-ag-alchemy-carrier-sandbox-rating.mjs
   railway run --service clawpilot --environment development -- node scripts/prove-ag-alchemy-carrier-sandbox-rating.mjs --execute --confirm=prove-ag-alchemy-carrier-sandbox-rating-v1
   ```

Every target-side persistence mutation re-locks the integration account with `FOR UPDATE` and reasserts the source-managed policy inside the same transaction. The provisioning command takes the matching target credential advisory locks, then reloads and locks the exact AG warehouse inside the serializable transaction, so a concurrent ordinary admin or origin mutation cannot cross the guarded establishment boundary.

Do not run this command against production or treat its freshly encrypted AG projection as production delegated authority. Production requires a separate, reviewed authorization record and explicit credential-owner, payer, ship-from, revocation, audit, and capability controls.
...
3. For a direct customer-managed connection, under **Billing accounts** add the carrier account number, sender name, and address registered to that account, then enable and select it. For an AG **EPISCS-managed sandbox rating** connection, select the already active managed account; the guarded delegation has projected warehouse `gwh5366613` into its read-only sender origin. The bounded Settings diagnostic is sender-billed; recipient- and third-party-billed execution require a later separately tested provider flow.
4. Review and, when needed, edit the U.S. test destination. The initial value is John Doe at `101 Academy Drive, Buzzards Bay, MA 02532`. The parcel remains one fixed `Test Product` measuring `12 x 10 x 6 in` and weighing `5 lb`. Correct a direct connection's origin on its selected billing account. Do not edit or override an AG delegated origin; correct warehouse `gwh5366613`, then rerun the guarded projection procedure after review.
...
8. Stop after rating for every connection that lacks `sandbox_label`, including an AG rating-only delegation. Only a separately authorized diagnostic-label connection, including AG's exact managed sandbox-fulfillment diagnostic profile, may choose one exact returned service and use the Settings procedure below or the order-bound procedure for an eligible packed order. Pickup, manifest, tracking mutation, and production actions remain prohibited.

## Settings Sandbox Rate, Label, Print, And Void Diagnostic

This procedure does not apply to AG's EPISCS-managed rating-only profile. It applies only after the guarded command installs the exact managed sandbox-fulfillment diagnostic profile, or to a separately authorized customer-managed connection with `sandbox_label`.

1. On a connection that separately authorizes `sandbox_label`, run the sandbox rate test above and retain the displayed `grq` evidence Global ID in the current no-store page. Rating does not return a label.
...
3. ClawPilot revalidates label capability, the destination fingerprint, the exact active account, and origin before writing a durable attempt. A rating-only or drifted managed connection fails without creating a label attempt. An authorized label command then writes a durable prepared attempt before network I/O and finalizes the returned label exactly once. If the outcome is `prepared` or `unknown`, stop; do not submit a new key.
4. Confirm the test-label Global ID, service, format, byte length, checksum, tracking reference, and `created` state. The current thermal diagnostic requests UPS ZPL or FedEx `ZPLII` with `STOCK_4X6`; ClawPilot normalizes the FedEx format name to `ZPL`, validates the decoded UTF-8 command envelope, and persists the returned bytes unchanged. Use **Download stored label** to retrieve those exact persisted PDF, PNG, or ZPL bytes. Existing PDF/PNG artifacts remain immutable and require a printer or print service that advertises that exact format and media. Never convert, scale, or inject margin commands into carrier label bytes. The checksum and length cover the actual bytes that will be delivered.
5. Choose an online compatible organization printer and select **Test print**. Compatibility requires the document format/media/type to be supported by both the printer profile and its enrolled agent, and the claiming runtime must repeat the same capability profile. This queues the already stored bytes and does not call rating or shipping again. A routing error is actionable printer or agent configuration feedback; it does not mean label creation failed.
6. Use print retry or controlled reprint for delivery recovery. Never create another carrier label to recover a print.
7. Select **Void test label** after inspection and provide a reason. ClawPilot uses the exact persisted carrier account and provider reference. A successful void changes the diagnostic label to `voided`; it does not delete its immutable label or print evidence. If UPS CIE returned the exact printable sample reference `1ZXXXXXXXXXXXXXXXX`, Settings instead presents **Close UPS sample without carrier call**. An owner or administrator with warehouse-execution permission records the reason and confirmation; ClawPilot writes `confirmed_no_active_label` evidence and retires the local diagnostic without calling UPS or representing the result as a carrier-confirmed void.
...
5. Final shipment packing slips use the versioned multi-page `packing-slip-letter-v2` renderer. Before label creation and tracking, package-specific warehouse paperwork is always a visibly provisional **Pack Work Instruction** using `pack-work-instruction-package-letter-v1`; it is never a packing slip or packing list. Both document paths retain SHA-256, byte length, safe filename, MIME type, template version, and immutable render snapshot, and neither may silently truncate an order line. The durable database and local print-agent compatibility identifier remains `packing_slip`, so operators and diagnostics distinguish a current Pack Work Instruction by `source_shipment_id IS NULL`, template `pack-work-instruction-package-letter-v1`, and render metadata `documentKind = pack_work_instruction`, `documentStage = pre_label_pack_work_instruction`, and `finalPackingSlip = false`; do not infer finality from that transport identifier. A package-scoped historical artifact without that template remains a **legacy pre-label packing list** for audit, is never relabeled as a Work Instruction, and may coexist with one newly rendered warned Work Instruction. Authorized users retrieve the exact stored ZPL, PDF, or PNG bytes from print-job details, and from a diagnostic label after its print artifact exists, through the active-organization artifact route. The route validates byte length and SHA-256, derives the safe MIME type and extension from the immutable format, returns a strong SHA-256 ETag, and requires every cached response to revalidate through the current signed-in active-organization authorization boundary; it never returns document bytes in JSON or logs. Downloading does not acknowledge delivery, convert media, call a carrier, or create a reprint. Local print-agent JSON claims preserve validated ZPL as `utf8` and carry binary PDF/PNG documents as `base64`; integrity values always describe the decoded bytes sent to the device.
...
1. Confirm the active organization and open **Settings > Integrations > Sales channels**. The Shopify store or Faire brand must be configured, verified, and product-readable. The verified connection is the authorization for automatic read-only product catalog sync; do not ask the customer for a second ClawPilot approval, page approval, or product approval. The interactive workbench is available only when `CLAWPILOT_COMMERCE_INTAKE_ENABLED=1` in development, local, or preview. With that flag in production, only the background catalog/order/image/inventory reconciliation lanes run, and only against active verified production accounts. Only an organization owner or user with **Manage operations** may use the development interactive controls.
2. Expand **Commerce intake · map, resolve, and promote** for the intended account. Confirm that a missing product policy initialized as resumed, then inspect the separate worker state. Do not treat policy initialization as proof that a job queued or started. Queueing requires an eligible runtime, current product-read scope and credential generation, current resumed policy revision, and an Operations product target in `shadow` or `active`. If activation is missing, `disabled`, or `read_only`, an authorized owner or administrator may select **Enable Shadow** to configure the organization-level product/review target; that is a data-target control, not a second provider or catalog authorization. Continue after the workflow reloads. If activation is `frozen` or changed concurrently, use **Review Operations** and resolve the incident or state change there; the intake shortcut must refuse the override. Confirm provider writes `0`, cursor advancement `no`, and that secrets, provider tokens, and cursor material remain absent.
3. Start with **Product catalog synchronization**. With every worker fence eligible, confirm status progresses from queued to running and completed without another user action. For Shopify, register the product create/update/delete subscriptions from the administrator workflow and verify the exact callback URI and all three topics. Change a product and confirm the signed receipt advances the catalog dirty watermark, triggers a complete read-only sweep, and advances only the dirty version captured by that sweep. Send a second event during a multi-page sweep and confirm a follow-up sweep remains queued until dirty and reconciled versions match. Set unmatched-product handling to **review** and confirm existing exact mappings still refresh while new unmatched variants remain held; set it to **auto-create** only when safe unmatched variants should become products automatically. Neither setting pauses provider reads. **Fetch product catalog**, **Fetch next product batch**, and **Check for product changes** remain bounded manual diagnostics, not required steps in the automatic full-catalog path. Do not use the order continuation for a product request. Before resolving exceptions, inspect the rendered provider and inventory-item identities, options, vendor/type, current and compare-at price, taxability/shipping flags, quantity, and weight. Shopify needs `read_products`; `read_inventory` is optional for the manual availability diagnostic and is not used by the background catalog worker. Faire needs `READ_PRODUCTS`; `READ_INVENTORIES` is optional for manual inventory hydration. A negative Faire quantity is valid provider evidence, while missing or `UNTRACKED` quantity must remain unknown rather than zero.
...
5. Select **Fetch operational orders**. The command starts a separate bounded order session and retains current-window provider orders across every lifecycle state; it does not create a canonical order. Open demand enters the resolution workflow. Fulfilled, cancelled, and closed rows remain visible as provider history and are ineligible for new ClawPilot fulfillment. Shopify is cutoff-fenced. Faire is explicitly a live cursor.
...
The primary **Operations > Orders** refresh is the normal ShipStation-style operator path. One click discovers bounded pages from every running connected store, reloads all retained provider rows, and performs exact status checks for existing canonical orders. A bounded continuation or provider failure is reported explicitly; it never presents a partial scan as complete. Provider issues remain attached to visible orders for resolution rather than hiding those orders in a separate candidate-only list. An order-level **Refresh from Shopify/Faire** reuses its request UUID after an uncertain response and must not create a second provider read.
...
10. Resolve only order lines that still need a product binding or manual price fallback. Fresh candidates automatically retain an exact nonnegative provider unit price in the order currency, including zero, as immutable `provider` resolution. Select an existing `gp` product or explicitly create one and bind the exact account-scoped provider variant. Enter a manual integer-minor-unit price and ISO currency only when provider money is missing, negative, cross-currency, or deliberately overridden; never substitute the Product master's mutable current price. For a candidate retained before this policy, use exact **Refresh** to create a current staged revision rather than rewriting the old evidence.
...
This procedure combines the leased development product-catalog worker with reviewed operational open-order intake. It is not a historical closed-order migration, inventory synchronization, or unattended order worker. `read_all_orders` allows an older still-open Shopify order to participate but does not authorize a closed-order backfill.
...
6. Observe only aggregate attempted, succeeded, failed, duplicate, invalid, and unsupported counts in the storefront status event. Address values, customer facts, carrier offers, and credentials must not enter that event, server logs, or the tenant policy.
7. Treat every background result as cache preparation only, and stop preparing when a timed Shadow policy expires or an **Until turned off** policy is edited to become ineligible or removed. It is not an order quote, does not select a service, and cannot guarantee the identity attached to a later callback. Reopen checkout on the selected address and require Shopify's live CarrierService callback to produce the authoritative store-entity, carrier, service, multi-package rate. If the callback omits the exact Customer GID, arrives after the policy is no longer eligible, cannot complete within its fixed budget, or cannot retain evidence for every configured carrier attempt, it fails closed.
   - Receipt claiming races pool acquisition against the callback cancellation signal and absolute deadline, releases a late connection without starting a transaction, checks transaction work cooperatively, and uses server-side statement timeouts to bound database work. Commit receives a final safety buffer; after commit resolves, its durable claim is returned. The claim may retry one whole idempotent database transaction for a transient serialization, deadlock, lock-timeout, or locally generated statement-timeout failure. External cancellation is not retried. A retry never repeats cartonization or a carrier call because claim happens before both. If the second attempt or the callback deadline fails, expect an authenticated empty-rate response plus a fixed safe reason code; inspect that code rather than database messages or checkout data.
8. Confirm a guest, another signed-in customer, an expired/removed/hidden policy, or a cart without the exact test variant cannot pass ClawPilot's Shadow callback gate. Shopify does not guarantee Customer GID in the CarrierService callback, and its successful-rate cache is customer-neutral, so neither saved-address warming nor repeated device tests prove deterministic customer enforcement. The Shopify customer search and ClawPilot policy save do not themselves prove storefront isolation: the setup must say `not_written` or `write_blocked` until the Delivery Customization and app-owned customer metafield are applied and provider-verified.
9. On an Active connection, confirm the global default is `show_all`: every eligible authenticated or guest cart receives the default customer-neutral service set. When provider enforcement is verified, authenticated `include_only`, `exclude`, or `hide_all` overrides filter that set by stable `clawpilot:<carrier>:<service>` code. Guests cannot receive a customer override because no durable Customer GID exists at that point.
10. On Shopify Advanced with this custom-distributed app, treat `DELIVERY_CUSTOMIZATION_FUNCTION_NOT_ELIGIBLE` as a hard enforcement blocker. Keep each live Shadow proof deliberately scoped; if **Until turned off** is used, explicitly edit or remove the policy when the proof is complete. Do not claim customer-specific service filtering is live. The supported paths are an eligible limited-visibility public App Store distribution or Shopify Plus, followed by provider activation and verification.

### Shopify Checkout Inventory Refresh

1. Use this worker with Postgres and `CLAWPILOT_COMMERCE_INTAKE_ENABLED=1`. Development, local, and preview retain their active sandbox/test setup. Production accepts only an active, verified production Shopify account; a sandbox, disabled, error, stale-generation, or unverified account is excluded before provider I/O. The account must have the effective inventory, product, and location read scopes and a checkout-ready CarrierService configuration. The same Railway poller process invokes `POST /api/integrations/commerce/inventory/process` with `PIPELINE_OUTBOX_WORKER_SECRET`; no browser session or separate user approval is required.
2. Confirm `/api/health` reports a fresh `shopifyInventoryRefreshWorker` heartbeat and `livenessStatus = reachable`. `operationalStatus = degraded`, stale accounts, dead/stale/overdue jobs, and live retries are account-scoped operational warnings; they do not make the global process health endpoint unavailable. The affected public callback still fails closed on stale inventory rather than reading Shopify inline.
3. The scheduler queues an account when its latest provider-fetched snapshot reaches half the configured maximum age. It claims one job at a time and renews the job and provider-read leases at bounded phases and every inventory page. Claim, projection, and completion recheck tenant, exactly active account, credential generation, activation revision, configuration row version, policy revision/hash, warehouse, effective scopes, unexpired lease, and the canonical readiness predicate. A changed fence aborts before local inventory mutation and cancels the old job; it must not complete under the replacement configuration.
4. The manager **Sync inventory** action remains recovery, not a second synchronization implementation. If a transient worker failure is retrying, let the leased retry run. If a current job is dead, correct the displayed scope, credential, location mapping, warehouse, or readiness blocker and run the manager command once with one stable idempotency key. A later successful current snapshot clears the dead-state recovery fence and permits automatic scheduling again.
5. Do not submit the manager command and worker repeatedly with new keys. Migration `0169` permits only one prepared inventory provider read per organization/account, and the orchestration also takes an account advisory lock. A captured attempt is replayed through projection; an active lease returns `SHOPIFY_INVENTORY_SYNC_IN_PROGRESS`; an uncertain provider outcome must be reconciled before another read.
6. Every successful or failed worker summary must retain `resource = inventory`, `readOnly = true`, `providerWrites = 0`, and `orderQuantityAdjustment = 0`. The worker may refresh ClawPilot's Shopify-authoritative operational position projection, but it cannot mutate Shopify inventory, import an order, reserve or consume order demand, advance an order cursor, or operate for Faire.
7. If a provider attempt and inventory run succeeded but the queue job is retrying with `SHOPIFY_INVENTORY_REFRESH_FAILED`, verify that the deployed completion query explicitly types every dynamic `jsonb_build_object` value, including the inventory-run Global ID. Preserve the succeeded run and let the same job replay it; do not force another Shopify read or delete evidence.
8. If the heartbeat is stale, verify the Railway poller process, `SHOPIFY_INVENTORY_REFRESH_POLL_MS`, worker secret, commerce feature gate, and migrations `0169`, `0171`, and `0172` before restarting. If leases are stale or jobs are overdue/dead, preserve the job and provider-attempt evidence; do not delete rows or bypass the queue. Migration `0172` lets the worker rotate an expired lease only when the immutable capture matches the exact organization, account, attempt, and request hash; recovery reprojects that capture without another Shopify read. Disable checkout quotes for the affected account until a current successful inventory run restores health.

### Faire Inventory Observation Polling

1. This read-only lane shares the authenticated inventory-process poller but executes and reports independently from Shopify inventory projection. Production accepts only an active, verified production Faire account; development retains its existing eligible account behavior. An OAuth credential must have requested `READ_INVENTORIES` to be scheduled, but that stored request is not verified grant evidence; each real selector GET is the fail-closed provider authorization proof. A verified generated single-brand key is likewise attempted against the read-only endpoint rather than presumed capable. The account also needs `shadow` or `active` Operations activation and at least one current mapped Faire variant. Product-intake policy revision and pause state are not read or changed by this worker.
2. Faire documents no inventory webhook. The worker therefore performs a complete scheduled sweep of exact mapped product-variant IDs, 50 selectors per request, using only the brand-profile and product-inventory GET endpoints. `/api/health` reports this as `faireInventoryPollWorker` with `eventTransport = scheduled_poll`, `webhookSupported = false`, and the exact scope, overdue, retry, stale-lease, terminal-failure, missing, and `UNTRACKED` counts.
3. Treat every retained quantity as `faire_channel_listing_observation`. Faire defines on hand as the listing quantity supplied by the brand, committed as units allocated to unfulfilled orders, and available as on hand minus committed. This is useful channel evidence, but it is not physical ClawPilot/WMS inventory authority, does not create an inventory position, does not satisfy fulfillment planning, and never overwrites a warehouse balance.
4. A missing provider row stays `missing`; `UNTRACKED` stays unknown; signed on-hand or available values are retained and never clamped to zero. The poller rejects inventory returned outside the requested selector set and fences each write against the exact channel-state row, mapping, credential generation, account state, and Operations activation revision. Its two provider GETs use four-second timeouts while shared database locks hold those authority rows, intentionally bounding an activation or credential change to less than eight seconds so no provider read starts beneath a superseded fence.
5. A stale lease may be reclaimed because repeating these GET requests has no provider-side effect. Retryable provider failures use bounded backoff per page; every successful continuation page resets that retry budget, so a healthy sweep is not limited to eight pages. A nonretryable failure or exhausted attempt budget becomes `dead` and blocks automatic replacement only for the same credential generation and activation revision. Correct the credential, scope, mapping, or provider condition, then use the authenticated `/api/integrations/commerce/faire/inventory` recovery command with the latest exact dead job, credential generation, error code, confirmation, and a reason of at least ten characters. Recovery is durably one-shot for that dead job and reason hash, preserves the dead evidence, and queues one new read-only sweep; it performs no provider call itself. Credential rotation or a new Operations activation revision fences old failures and permits a new current sweep.
6. Every worker and recovery result must continue to report `providerWrites = 0` and `wmsProjectionApplied = false`. Migration `0223` is required. A fresh heartbeat plus a completed local or hosted poll proves only the observation lane; production activation and live-provider acceptance still require the configured Faire credential to return brand identity and inventory for a real mapped variant.
...
13. Configure physical printers under **Printing**, import carrier bills under **Carrier invoicing**, and review shipment-to-shipper assignment, retained Markup Directive (MUD) pricing, and GL outputs under **Shipment pricing & GL**. Those are separate facility services and do not become configured merely because a warehouse exists.
...
## Canonical Plan Acceptance And Existing Warehouse Execution

1. Open **Operations**, select **Orders**, and open a non-mock commerce order in `imported` state. It must have been promoted from one reviewed Shopify or Faire candidate. The hosted workbench does not create proof orders; deterministic mock generation remains reserved for automated tests.
2. Before selecting **Plan order**, obtain the permanent `gcte` Global ID for one sealed `operational` cartonization/rate aggregate belonging to that exact promoted candidate and its one active warehouse. Assumption-backed sandbox evidence, a candidate with a changed provider source hash, a stale material/recipe/profile revision, or an incomplete line-to-package allocation is not acceptable warehouse evidence. Shadow may accept operational pack facts paired with explicitly retained sandbox carrier-read estimates; Active planning fails closed unless the sealed aggregate records a `production` carrier-read environment.
3. Verify that every line quantity is assigned exactly once across the evidenced packages and that every whole-shipment carrier offer covers the complete ordered package-key array in the order currency. The selected rate must use one UPS or FedEx service for all packages; never combine services package by package. When a requested-delivery timestamp exists, every selected offer must meet it.
4. Verify inventory and packaging-material authority before planning. For Shopify authority, the evidence must identify one exact latest successful sync run and projected product/location level, and enough provider-committed quantity must remain unclaimed by another active order. Planning records one `provider_commitment` claim only; it does not split provider authority across levels, increment the Shopify-projected reserved balance, append a second local inventory-ledger delta, or write to Shopify. For ClawPilot authority, a line may draw from more than one eligible position when no single pick face or reserve position covers it. Candidate positions are locked and consumed in location pick-sequence then permanent position Global-ID order; each selected position receives its own `local_balance` reservation, matching reserved-balance/ledger delta, and fulfillment allocation, and their quantities must sum exactly to the line. Faire catalog/list availability is not inventory authority; a Faire order needs eligible ClawPilot-authoritative inventory at this boundary. Every selected carton or mailer also needs enough warehouse on-hand after subtracting other active plan claims. Planning locks that stock and creates one exact active claim per plan/material/warehouse; it does not decrement physical packaging on-hand. Active mode fails closed when a planned or released order lacks its sealed production carrier-read evidence. A later guarded command must consume or release each claim exactly once.
5. Select **Plan order**, enter the exact cartonization evidence Global ID and a specific planning reason, then submit once. The browser retains one idempotency key for a safe replay of that exact order revision, evidence reference, and reason. If the order or evidence changed, reload and review the current facts instead of changing the key to force execution.
6. On success, confirm the order is `planned` and review the fulfillment-plan Global ID, evidence Global ID, warehouse, exact physical package count and allocations, selected carrier/service, carrier-cost estimate, actual checkout shipping charge when present, and signed checkout-to-estimate variance.
7. Keep those monetary facts separate. The checkout charge is what the customer paid; the selected rate is a pre-label carrier estimate; their variance is not carrier-billed actual. No MUD may be calculated until a carrier-billing CSV creates an approved exact shipment match and an applicable approved `actual_cost` directive.
8. Confirm the planning command created no label, postage purchase, shipment, packing slip, tracking observation, commerce-fulfillment export, provider inventory mutation, or commerce write. A `planned` status is not proof that anything shipped.
9. Continue to **Release to warehouse** only after reviewing the plan's inventory authority. Shopify provider commitments are supported at release/pick/consumption boundaries without a second local balance; execution-time rerating, multi-package label creation, and provider fulfillment dispatch remain separate boundaries.
10. Resolve every open high or critical exception before release. Do not bypass an incomplete reservation or allocation with direct SQL.
11. Use **Release to warehouse**, record a specific operational reason, and submit once. The client keeps one idempotency key for safe retries of that release attempt. For Shopify authority, release serializes with inventory reconciliation and revalidates every active provider claim against the still-latest exact successful sync and projected balance before creating the wave. If that evidence changed, refresh inventory and replan.
12. On success, confirm the order and selected plan are `released`, exactly one released wave exists, and exactly one `ready` pick task exists for every fulfillment allocation. A line split between pick-face and reserve stock therefore has multiple tasks even though it remains one order line.
13. Verify that every expected task is ready and that the displayed pick count matches the allocation count. Task sequence must follow source-location pick-route order, then the permanent position Global ID, so a retry cannot silently reorder split picks. **Confirm all picks** is intentionally unavailable for partial, short, blocked, or already confirmed work.
   - If newer Shopify inventory evidence no longer supports the active provider commitment, the drawer replaces **Confirm all picks** with **Reconcile Shopify fulfillment**. The command performs a fresh bounded Shopify read and proceeds only when one exact successful fulfillment, created after warehouse release, covers every canonical line at the released location. It retains immutable evidence, cancels the wholly unpicked tasks/wave/plan/order, and releases provider and packaging claims. It does not write to Shopify, create a ClawPilot shipment or fulfillment export, or notify the customer again. A partial, pre-release, wrong-location, cancelled, changed, already-picked, labeled, shipped, or otherwise non-exact order fails closed.
14. Use **Confirm all picks**, record a specific operational reason, and submit once. The client keeps one idempotency key for safe retries. The command rechecks the exact order version, released plan and wave, all ready picks, active organization, inventory positions, and blocking exceptions before changing state.
...
23. When shipment confirmation is reached through an eligible production label path, a Shopify provider commitment moves to `consumed` without mutating the Shopify-projected local position or ledger; the queued commerce fulfillment export remains the provider-write path. A terminal provider commitment cannot be reactivated. The exact post-commit Shopify and Faire fulfillment/tracking dispatchers, operator reconciliation, and bounded scheduled recovery are implemented, but execution still requires their current provider authority, Active capability claims, immutable shipment evidence, and runtime configuration. Shopify uses the exact account notification policy captured at confirmation, with an audited per-order exception; Faire retailer notification remains provider-managed. The only released-plan cancellation exposed here is the exact, wholly unpicked Shopify external-fulfillment recovery described above. Generic abandonment, partial-fulfillment recovery, and any cancellation after a pick remain unavailable; do not perform them by direct SQL. Active-mode arbitrary production carrier label/tracking creation and post-label packing-slip completion remain separate boundaries. Generic provider-agnostic commerce export, continuous reverse catalog synchronization, multi-warehouse splitting, pickup scheduling, manifests, production carrier actions, scanner claims, per-task scans, and short-pick recovery remain separate later slices.
...
1. On a narrow screen, use the horizontally scrollable tab row beneath the Operations header to reach **Orders**, **Exceptions**, **Warehouses**, **Carrier invoicing**, **Shipment pricing & GL**, and **Printing**. Swipe the tab strip or use its labeled left/right controls; the controls disable at the corresponding edge.
...
- For Shopify authority, compare the account dirty and reconciled inventory-refresh versions, latest verified inventory webhook receipt, queued/processing/failed refresh job, provider-attempt start time, and latest succeeded inventory-run Global ID. Never treat webhook quantity as the reconciled balance.
- For Phase-one targetability evidence, compare the exact receipt and dirty version to its append-only target signal, then inspect targeted/full-required reason and delivery-lag metrics. A targeted classification is not evidence that a partial provider read ran; confirm the complete inventory run and capture as usual.
...
- For Shopify, use **Sync inventory** or the leased refresh worker for a fresh authoritative read. Do not edit the watermark, copy webhook quantity into a position, or bypass checkout's dirty-version fence. Confirm dirty equals reconciled and the acknowledged provider attempt began after its triggering job claim.
