...
7. Treat every background result as cache preparation only, and stop preparing when the selected Shadow policy expires. It is not an order quote, does not select a service, and cannot guarantee the identity attached to a later callback. Reopen checkout on the selected address and require Shopify's live CarrierService callback to produce the authoritative store-entity, carrier, service, multi-package rate. If the callback omits the exact Customer GID, arrives after policy expiry, cannot complete within its fixed budget, or cannot retain evidence for every configured carrier attempt, it fails closed.
...
10. On Shopify Advanced with this custom-distributed app, treat `DELIVERY_CUSTOMIZATION_FUNCTION_NOT_ELIGIBLE` as a hard enforcement blocker. Keep the Shadow proof bounded and do not claim customer-specific service filtering is live. The supported paths are an eligible limited-visibility public App Store distribution or Shopify Plus, followed by provider activation and verification.
