...
3. Start with **Product catalog synchronization**. With policy resumed and every worker fence eligible, confirm status progresses from queued to running and completed without another user action. Use pause only to stop future provider reads and automatic creation; resume makes the account eligible for a full reconciliation but does not claim queueing until every worker fence passes. **Fetch product catalog**, **Fetch next product batch**, and **Check for product changes** remain bounded manual diagnostics, not required steps in the automatic full-catalog path. Do not use the order continuation for a product request. Before resolving exceptions, inspect the rendered provider and inventory-item identities, options, vendor/type, current and compare-at price, taxability/shipping flags, quantity, and weight. Shopify needs `read_products`; `read_inventory` is optional for the manual availability diagnostic and is not used by the background worker. Faire needs `READ_PRODUCTS`; `READ_INVENTORIES` is optional for manual inventory hydration. A negative Faire quantity is valid provider evidence, while missing or `UNTRACKED` quantity must remain unknown rather than zero.
