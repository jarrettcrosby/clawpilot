...
7. If the heartbeat is stale, verify the Railway poller process, `SHOPIFY_INVENTORY_REFRESH_POLL_MS`, worker secret, commerce feature gate, and migrations `0169` and `0171` before restarting. If leases are stale or jobs are overdue/dead, preserve the job and provider-attempt evidence; do not delete rows or bypass the queue. Disable checkout quotes for the affected account until a current successful inventory run restores health.
