...
The bounded Railway-development sandbox-rating delegation from Express Parcel International DBA EPISCS to AG Alchemy, LLC is an explicit target projection, not adapter fallback: EPISCS remains the credential owner and its integration/account identities, ciphertext, billing metadata, and verification state remain unchanged. The guarded establishment command freshly verifies each EPISCS source credential, creates fresh AG integration/account identities and AG-authenticated AES-GCM ciphertext, and projects AG warehouse `gwh5366613` as the target account's sender origin.

Those AG accounts are `operator_owned`, retain auditable EPISCS source references, and default to `authorizationScope: 'sandbox_rating_only'` with only `allowedCapabilities: ['sandbox_rate']`. Credential reveal, source-managed credential/account mutation, disconnect, generic connection verification, label, void, pickup, manifest, shipment, and every provider-mutation path fail closed. Missing or extra managed fields disable the lane rather than inheriting legacy capabilities. The generic **Test connection** command is rejected before provider I/O and cannot change verification state; **Test sandbox rate** is the sole executable provider diagnostic in that default profile. Every target-side persistence mutation locks the integration account with `FOR UPDATE` and reasserts the managed policy in the same transaction. Provisioning verifies the compiled development database identity, uses matching target credential advisory locks, and reloads plus locks warehouse `gwh5366613` inside the serializable transaction. The command is plan-first, rating-only apply requires the literal `establish-ag-alchemy-carrier-sandbox-v1` confirmation, and an unchanged rerun is a no-op.

The same guarded command has one explicit test-label profile: `--enable-sandbox-fulfillment --apply --confirm=enable-ag-alchemy-carrier-sandbox-fulfillment-diagnostics-v1` installs exactly `authorizationScope: 'sandbox_fulfillment_diagnostic'` and `allowedCapabilities: ['sandbox_rate', 'sandbox_label']`. It permits provider-sandbox rate, confirmed label create, stored-file download, compatible-printer test print, void, and UPS CIE local sample close while leaving credentials and full billing identity concealed. It never permits production label, shipment, pickup, manifest, tracking, or commerce writes. Omitting that flag on the next guarded run intentionally restores rating-only policy; UI and server checks then deny label creation and all stored-artifact actions. The separate rating proof accepts either exact managed profile, revalidates the current warehouse sender name/address, produces exactly one UPS and one FedEx quote, and asserts no label-attempt, label, shipment, or print mutation. This does not prove production delegated authority, billing, rating, or shipping; a production design must model the delegation grant, credential owner, payer, ship-from warehouse, revocation, audit, and capability policy as first-class records without relying on replicated ciphertext.
...
Rating and shipping remain separate provider commands. A successful rate contains no label document or tracking number. Only a connection that authorizes `sandbox_label`, including the exact managed sandbox-fulfillment diagnostic profile, permits an authorized operator to select one evidenced service and choose a provider-native 4 x 6 output before calling the provider sandbox Ship API through a durable prepare/call/finalize command. The default AG rating-only profile does not authorize this path. The standard UPS Shipping diagnostic exposes native `ZPL`; the current UPS OpenAPI describes `GIF`, `ZPL`, `EPL`, and `SPL` for standard shipment labels and does not establish native PDF or PNG for this path. ClawPilot therefore does not advertise UPS PDF/PNG or silently translate GIF. FedEx exposes native `ZPLII` with `STOCK_4X6`, or native `PDF`/`PNG` with `PAPER_4X6`. ClawPilot validates the decoded format and dimensions where applicable, records the exact provider bytes and source metadata, and only then permits a compatible print or stored-file download route. The associated void resolves the exact persisted account, and an ambiguous create or void result blocks a fresh attempt until reconciliation.
