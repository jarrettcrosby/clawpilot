...
- This checkpoint is bounded catalog mapping plus day-to-day nonterminal order intake. It is not a closed-order historical import, inventory synchronization, or an unattended catalog/order worker. Shopify `read_all_orders` is required only so an older order that remains open can participate.
- Provider writes, unattended Shopify order/customer receipt processing, Faire scheduled polling, Shopify multi-merchant OAuth, zero-downtime dual-secret rotation, webhook registration, cursor reconciliation/replay workers, and fulfillment export remain Phase 6 exit work.
