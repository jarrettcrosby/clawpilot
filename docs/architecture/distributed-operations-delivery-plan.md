...
- Shopify signed receipt intake may be explicitly enabled only after API verification and a valid signed app-scope, product, or inventory delivery proves the exact credential generation. Order/customer webhook processing remains disabled.
- Migration `0114` and the authenticated Sales Channels workflow provide two operator-controlled Shopify/Faire checkpoints: bounded product-catalog mapping intake and bounded operational-order normalization/resolution/promotion. Catalog candidates require an exact choice to map an existing CRM product, create an integration-account/provider/variant-scoped CRM product and mapping, or record an audited exclusion. Each unresolved order line requires an exact product choice or explicit creation plus a confirmed nonnegative order-time price in minor units and ISO currency. Customer, ship-to, requested delivery, and package facts also require executable decisions.
- Migration `0115` durably prepares provider-read intent before every initial, exact-retry, or exact-refresh network call and adds resource-scoped immutable batch lineage, encrypted server-only continuations, and first-class rejections. Retrying the same request key reuses its account, credential generation, resource, query/window, and target. Separate UI cards start and continue product and order sessions; a product continuation cannot continue orders or vice versa. The browser receives only a prior intake-run Global ID, never cursor material.
- Order paging keeps one Shopify provider-time cutoff through the batch chain. Faire exposes a live cursor, so list candidates require an exact-order refresh before validation and a later **Check for newer orders** pass. Consumed continuation material is cleared, and invalid, expired, exhausted, superseded, resource-mismatched, or credential-mismatched continuations fail closed with a visible reload or restart action.
- The checkpoint commands include separate order/catalog fetch and continuation, catalog product resolution, exact order-rejection retry, audited rejection exclusion, order-line/product/customer/address/delivery/package resolution, exact order refresh, validation, unsupported acknowledgement, and promotion. They require **Manage operations**, active-organization object fences, current row versions, stable request-hash-aware idempotency keys, exact-result receipts, protected candidate retention, and redacted audit evidence.
- Each normalization failure becomes a first-class row-versioned rejection. **Retry exact order** reads the rejected order identity again; **Exclude record** records a safe reason and terminal disposition. Product rejections require exclusion followed by provider correction and another catalog read because the adapters do not expose a safe exact-variant read. A staged stale order has **Refresh**; missing facts have explicit bind/create/confirm/manual actions; and a permanently unsupported staged order has **Mark unsupported**.
- Shopify promotion uses only positive exact `unfulfilledQuantity` as canonical line demand. The canonical merchandise total is remaining quantity multiplied by operator-confirmed order-time price; the source payload retains provider subtotal, canonical total, signed variance, and reconciliation-policy version.
- This checkpoint is bounded catalog mapping plus day-to-day nonterminal order intake. It is not a closed-order historical import, inventory synchronization, or an unattended order worker. Shopify `read_all_orders` is required only so an older order that remains open can participate.
- Generic provider writes, unattended Shopify order/customer receipt processing, Shopify multi-merchant OAuth, zero-downtime dual-secret rotation, unsupported webhook groups, general order/inventory cursor reconciliation, continuous reverse catalog synchronization, and provider-agnostic fulfillment export remain Phase 6 exit work. Exact post-commit Shopify and Faire fulfillment/tracking dispatchers, operator reconciliation, and bounded scheduled recovery are implemented behind current provider authority, Active capability claims, immutable shipment evidence, and runtime configuration; they are not a generic export path or production-activation proof. Shopify customer notification follows its explicit account policy with an audited per-order exception, while Faire retailer notification is provider-managed. The development-only product catalog worker is a narrower product-read exception and is not production-domain activation evidence. A verified product-readable connection authorizes that worker without a second approval and initializes a missing policy as resumed in the eligible runtime; policy initialization is not proof that work queued or started. A durable job queues and executes only after the runtime, product-read scope, current credential and policy revisions, and Operations product target are eligible.
- The checkpoint remains development-only behind `CLAWPILOT_COMMERCE_INTAKE_ENABLED=1`; production is refused. Contract/build evidence does not count as hosted Shopify or Faire acceptance.
...
| S29 | Order and product pagination are server-owned, replay safe, resource scoped, and honest about provider consistency. | 6 | Adapter + Postgres concurrency: matching `fetch-next` commands consume one encrypted continuation through the prior run Global ID, never return cursor material, and reject cross-resource/tenant/query/credential/expired reuse. Shopify proves cutoff-fenced paging. Faire orders remain a live cursor with exact refresh and later reconciliation. |
| S30 | One malformed provider record does not discard valid records and never becomes an advisory-only dead end. | 6 | Normalizer + UI integration: valid candidates stage; the rejection has Global ID, row version, external ID/code/safe message; exact order retry can stage or replace it; audited exclusion is available for every rejection; product rejection directs exclusion, provider correction, and catalog refetch. |
| S31 | Operational intake does not become history, inventory synchronization, or an unattended catalog mirror. | 6 | Adapter contract: only nonterminal orders enter order candidates; bounded product/variant pages create mapping candidates; no inventory ledger or durable sync cursor changes; an older still-open Shopify order remains eligible when required scopes are present. |
| S32 | Every provider read has durable pre-call authority. | 6 | Postgres + fault-injection: initial fetch, exact retry, and exact refresh commit a fixed account/resource/credential/query/window/target intent before the adapter call; next-page reads require the preceding resource-scoped continuation; uncertain retry with the same key reuses its authority; conflicting key reuse fails; successful stage links the intent once. |
| S33 | Product candidates have complete local dispositions. | 6 | Command + UI integration: current row-version decisions map an existing active `gp`, create an integration-account/provider/variant-scoped CRM product and exact mapping, or record an audited exclusion; retries return the exact receipt and cross-account identities fail. |
| S34 | Partially fulfilled Shopify orders create only remaining work. | 6 | Adapter + promotion integration: exact ordered/current/fulfilled/unfulfilled quantities reconcile per line; zero-remaining lines are excluded; canonical line quantities equal only positive unfulfilled quantities. |
| S35 | Canonical merchandise money is reconciled to remaining work. | 6 | Promotion integration: canonical total equals the sum of remaining quantity times confirmed order-time unit price; source evidence retains provider subtotal, canonical total, signed variance, and reconciliation-policy version. |
