...
- Provider writes, unattended Shopify order/customer receipt processing, Faire scheduled order polling, Shopify multi-merchant OAuth, zero-downtime dual-secret rotation, webhook registration, order/inventory cursor reconciliation and replay workers, and fulfillment export remain Phase 6 exit work. The development-only product catalog worker is a narrower product-read exception and is not production-domain activation evidence. A verified product-readable connection authorizes that worker without a second approval and initializes a missing policy as resumed in the eligible runtime; policy initialization is not proof that work queued or started. A durable job queues and executes only after the runtime, product-read scope, current credential and policy revisions, and Operations product target are eligible.
