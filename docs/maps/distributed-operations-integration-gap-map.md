...
| Postgres transactions | `query`, `withTransaction`, advisory locks, foreign keys, check constraints, and unique keys already exist. Hosted operations fail closed without Postgres. The explicit warehouse-release command atomically revalidates readiness, releases the selected plan, creates one released wave and ready pick tasks, advances the order version, and records event/audit evidence. The bulk all-ready pick command locks affected inventory positions, revalidates the released execution aggregate, confirms every ready pick, completes the wave, advances the order, and records ledger/event/audit plus exact replay-result evidence. Legacy hosted mock orders are archived through an additive migration that releases reservations while preserving immutable evidence. | Put each aggregate mutation, ledger evidence, domain event, audit event, outbound intent, and receipt transition in one transaction. | **Implemented development slice through bulk all-ready pick confirmation.** Production order ingestion, provider-attempt boundaries, scanner/per-task pick, short-pick, label, print, ship, and production-provider mutation services remain gaps. |
...
| UI system | MUI, authenticated hash-based sections, desktop/mobile navigation, responsive work surfaces, and shared timezone formatting are established. The Operations workbench includes order and exception drill-ins plus explicit release and confirm-all-picks dialogs on desktop and mobile. | Reuse shell, typography, icons, tables, drawers, dialogs, accessibility, and workspace switching. | **Implemented first workbench.** Scanner/offline workflows, pack station, advanced wave planner, portal, and broader exception operations remain gaps. |
| Integrations and secrets | Organization-scoped Toast credentials use AES-GCM; per-user Maton and QuickBooks bindings are validated; settings and workers avoid returning secrets. Direct UPS/FedEx/USPS credentials are organization- and environment-scoped, encrypted, verified, and never returned. UPS and FedEx expose one fixed sandbox-only rating action with normalized results and append-only redacted evidence. Deterministic commerce/carrier/print mocks remain internal tests only. | Store only a validated credential reference on an operations integration account; adapters resolve the secret server-side. | **Implemented credential boundary and constrained UPS/FedEx sandbox rating.** Production rating, webhook verification, transactional carrier actions, token cache, provider-attempt reconciliation, and certification remain gaps. |
| Observability and recovery | `/api/health`, `/api/persistence/status`, worker queue health, structured audit events, Railway backups, logical exports, and release gates exist. Operations health now reports migration state, command failures/stale processing, and active/shadow organization counts. | Add queue, adapter, ledger reconciliation, optimizer, print, and shipment checks to those surfaces. | **Partial operations health.** Metrics, traces, alerts, dashboards, reconciliation evidence, and drills remain gaps. |
...
The native shell registration, migration and command health gate, internal route, UI, scoped organization activation, planned-order review, explicit warehouse release, deterministic bulk all-ready pick confirmation, fixed UPS/FedEx sandbox rating, redacted rate evidence, and committed test suite exist on `dev`. The hosted UI no longer creates mock orders, and migration `0088` archives legacy mock artifacts without deleting immutable history. Both warehouse execution commands require the current row version, a reason, a stable idempotency key, an eligible activation state, and no open high/critical exception. Release additionally requires complete reservations and allocations; pick confirmation requires one released plan and wave plus every expected task in `ready`. There is still no operations worker target, production commerce order ingestion, scanner/per-task command path, complete operations health section, OR-Tools implementation, public checkout contract, or certified production carrier action. The module remains pre-activation outside the bounded development proof.
...
2. Extend the atomic proof-order command pattern to production order, inventory, warehouse, carrier, print, and billing commands.
...
10. Add provider routes, workers, complete operations health/reconciliation metrics, and integration/warehouse activation subscopes.
