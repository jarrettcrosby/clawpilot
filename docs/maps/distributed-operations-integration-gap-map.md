...
| Postgres transactions | `query`, `withTransaction`, advisory locks, foreign keys, check constraints, and unique keys already exist. Hosted operations fail closed without Postgres. The planned proof command atomically writes order, reservation, plan, package/rate evidence, events, audit, and a request-hashed command receipt. The explicit warehouse-release command atomically revalidates readiness, releases the selected plan, creates one released wave and ready pick tasks, advances the order version, and records event/audit evidence. The bulk all-ready pick command locks affected inventory positions, revalidates the released execution aggregate, confirms every ready pick, completes the wave, advances the order, and records ledger/event/audit plus exact replay-result evidence. | Put each aggregate mutation, ledger evidence, domain event, audit event, outbound intent, and receipt transition in one transaction. | **Implemented development slice through bulk all-ready pick confirmation.** Provider-attempt boundaries and scanner/per-task pick, short-pick, pack, label, print, ship, and production-provider mutation services remain gaps. |
...
- `app_src/lib/persistence/operations.ts`: tenant-scoped reads, canonical CRM catalog projection, deterministic provider-customer resolution, exact-result command receipts, planned and explicitly shipped mock proof modes, atomic operator-controlled warehouse-release and bulk all-ready pick-confirmation commands, activation control, and audited exception dispositions;
