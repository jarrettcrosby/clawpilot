...
- **Foundation draft** means untracked migration `db/migrations/0081_distributed_operations_foundation.sql` proposes the named Postgres structure. It is not proof of an applied migration or working runtime.
- **Working-tree draft** means parallel, untracked operations code exists in the shared worktree but has not established a committed, tested, deployed contract.
...
| --- | --- | --- | --- |
| Application architecture | Next.js App Router and React live under `app_src`; route handlers are under `app_src/app/api`; domain and persistence code is under `app_src/lib`; MUI is the component system. Parallel draft code now uses `app_src/lib/operations` and `app_src/lib/persistence/operations.ts`. | Add operations as a native section, route family, domain service, and persistence adapter in those boundaries. | **Existing platform plus working-tree draft.** There is no plugin runtime or independent operations application boundary. |
...
| Postgres transactions | `query`, `withTransaction`, advisory locks, foreign keys, check constraints, unique keys, and optimistic version patterns already exist. Hosted runtimes fail closed without Postgres. A working-tree persistence draft reads an operations workspace and order details from proposed tables. | Put aggregate mutation, ledger evidence, domain event, audit event, and outbound intent in one transaction. | **Existing platform plus read-model draft.** Atomic reservation, command-receipt, and complete mutation services do not yet exist. |
...
| API conventions | Node.js Next route handlers authenticate with `requireRequestUser`, validate bounded JSON, return structured error codes, use `Cache-Control: no-store`, and require caller-stable idempotency keys for consequential writes. | Expose `/api/operations/v1/...` through route handlers and keep provider payloads behind adapter schemas. | **Existing style.** No operations routes or public checkout contract exist. |
...
| Authorization | Global role, organization membership role, explicit permission JSON, hierarchy, and resource sharing are separate controls. Parallel edits add typed `viewOperations`, `manageOperations`, and `executeWarehouse` flags, role defaults, settings controls, and an operations capability helper. | Enforce typed operations permissions server-side before reading or mutating tenant data, then narrow sensitive commands with explicit capabilities. | **Working-tree draft.** The three coarse flags are not the requested fine-grained matrix, and no operations route enforcement exists yet. |
...
| Reporting | Pipeline, CRM, POS, Accounting, Dashboard, and Versions each implement purpose-built Postgres projections and responsive UI. A working-tree operations persistence draft queries summary, order, package, rate, event, and billable views. | Build operations read models and metrics from native events and snapshots; do not use Sheets as an operations reporting source. | **Pattern plus read-model draft.** KPI definitions, durable projection jobs, retention/aggregation, and health metrics do not exist. |
...
Parallel implementation present at the final documentation inspection includes:
...
- `app_src/lib/persistence/operations.ts`: tenant-scoped workspace/order read queries and draft imports for a mocked proof flow.

These files are untracked or modified parallel work. At inspection time there is no operations route, shell registration, UI, worker target, health section, committed test suite, OR-Tools implementation, or production provider verification. Their presence narrows Phase 1/2 implementation work but does not change this document set from pre-activation design.

## Foundation Migration Coverage

The foundation draft is additive and organization-scoped. Its composite foreign keys generally bind operations rows to the active workspace, pipeline, warehouse, location, customer, and product. The following table is a design review, not migration acceptance.
