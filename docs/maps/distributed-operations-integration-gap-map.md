...
| Postgres transactions | `query`, `withTransaction`, advisory locks, foreign keys, check constraints, and unique keys already exist. Hosted operations fail closed without Postgres. The mock vertical slice atomically writes order, reservation, ledger, events, audit, billable facts, outbox intent, and a request-hashed command receipt. | Put each aggregate mutation, ledger evidence, domain event, audit event, outbound intent, and receipt transition in one transaction. | **Implemented development slice.** Provider-attempt boundaries and production mutation services remain gaps. |
...
- `app_src/lib/persistence/operations.ts`: tenant-scoped reads, canonical CRM catalog projection, deterministic provider-customer resolution, command receipts, a complete mocked proof transaction, activation control, and audited exception dispositions;
...
The native shell registration, migration and command health gate, internal route, UI, scoped organization activation, and committed test suite exist on `dev`. There is still no operations worker target, complete operations health section, OR-Tools implementation, public checkout contract, or production provider verification. The module remains pre-activation outside the bounded development proof.
