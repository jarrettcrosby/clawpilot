...
- `Calculations` and `Dashboard` are deterministic generated reports. Provisioning clears only their generated range, replaces stale embedded Dashboard charts with the managed chart set, and never clears the operator-owned Opportunities table or an established Dropdown catalog.
