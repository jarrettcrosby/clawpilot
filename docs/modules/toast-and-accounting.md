...
3. Normalize menu, group, item, category, SKU, PLU, description, primary HTTPS image reference, price, visibility, and active state without storing order or customer identity or copying image bytes into Postgres. The large POS catalog response exposes only description/image availability flags; it loads one selected item's detail on demand and serves its image through an authenticated, organization-scoped, same-origin proxy that revalidates the provider URL and image bytes.
...
- Suggested targets are explicitly unsaved previews. **Save suggestion** persists one catalog-backed row immediately; **Save N suggestions** persists only suggestions shown by the current search. Neither action requires changing away from the proposed value and neither posts to QuickBooks. Missing/retired catalog targets remain blocked. Manual changes keep their separate **Save** action. Server-confirmed mappings show **Saved**, and saving one suggestion or the posting profile preserves unrelated unsaved edits in the same organization, location, and configuration scope. Regenerating a business date remains an explicit next step.
- An unmatched Toast sales item can prepare an immutable QuickBooks product draft from its menu name, SKU or PLU, description, price, and location context. When Toast supplies an image, POS marks it as available in the mapping list and loads it only after the operator opens that product's draft review through the authenticated same-origin proxy; this bounds provider image work to the selected item. The browser never receives the external Toast image URL, ClawPilot does not persist the binary, and the image is not attached to the QuickBooks product draft. The operator may assign an existing active QuickBooks category, including a nested category shown by its fully qualified path. Categories are parent containers and are never offered as ordinary Toast item mapping targets. Product creation follows the normal submit, approval, provider-write, and catalog-refresh controls before the new item can be selected as a mapping target.
...
- One durable issue state is maintained per organization, restaurant, and business date. Its latest issue fingerprint is refreshed in place, so repeated worker checks, changed blockers, and recurrence after resolution do not create another email slot for that day.
- Delivery waits until the required Sales Receipt and Journal Entry posting outcomes have settled, then combines the remaining failures into one email. A journal-only Payment Exceptions date waits only for its required journal outcome.
...
- Email delivery reserves one durable slot per eligible recipient, restaurant, and business date and rechecks the latest issues, posting state, and recipient authorization immediately before sending. A resolved, externally posted, cancelled, or no-longer-authorized delivery becomes terminal `suppressed`. A provider failure or ambiguous send outcome becomes terminal `dead` and is not retried, preventing a second email after an uncertain first attempt.
- Recipients are active owners and active organization administrators who hold both accounting-view and access-management permissions for that exact organization. Descendant organizations, global role alone, and inactive memberships do not confer recipient access.
- `/api/health` exposes pending, failed, dead, stale, and overdue accounting-notification deliveries. Suppressed deliveries remain durable audit evidence but are not counted as unhealthy queue work. The Toast worker reconciles stale open issues independently from ingestion so a mail-provider failure cannot poison POS projections.
...
16. Enable **Email issue alerts**, leave confirmed Sales Receipt and Journal Entry posting failures for one business date, and confirm the worker waits for both outcomes before creating one combined email for each eligible recipient. Run the worker again, change the issue set, resolve it, and reintroduce it; confirm none of those actions creates a second email slot for that recipient, location, and date.
17. Resolve or externally post the documents after the delivery slot is reserved and confirm the outbox becomes `suppressed`. Force an ambiguous provider send and confirm it becomes terminal `dead` without retry.
18. Disable email alerts and confirm issue Activity continues without an outbox row. Confirm a demo or reserved recipient is rejected even if it has owner permissions.
19. Open the full-range historical parity view and verify matched, unmatched, and ambiguous receipt/journal evidence is organization scoped, read-only, and free of raw provider payloads.
20. Create a paid future order, confirm it appears on the payment date with its fulfillment date, and verify the payment-date journal credits Payment Exceptions while the fulfillment-date documents release Payment Exceptions without duplicating sales.
21. Force a missing mapping, open check, out-of-balance preview, incomplete fee batch, missing source date, overdue Toast job, Toast sync failure, and QuickBooks posting failure. Confirm each appears in the posting queue, the issues-only filter retains it, and its action opens the exact corrective surface.
