...
- Google sign-in is additive to magic codes and linked separately by each existing ClawPilot user from web Settings > Security or the iPhone Session Security card. The platform-owned Google Web OAuth client must be configured, the verified Google email must exactly match the signed-in ClawPilot user email, and the durable Google subject must remain unique to that same user. One user's Google link never authorizes another user. ClawPilot persists the subject-to-user link, never the ID token. A later Google login resolves only that exact stored subject plus verified email, requires the ClawPilot user and at least one direct organization membership to remain active, and selects one of that user's direct memberships. The linked user may switch among all of their direct active memberships regardless of legacy organization Google-policy flags; organization hierarchy, root ownership, or another user's link never supplies implicit access. A Google-authenticated session still cannot create a new root business or membership. Google authentication never creates a user, accepts an invitation, creates a membership, changes a role, or links a different email. `app_organization_auth_policies` is retained only as inert compatibility data while released clients migrate; it no longer grants or removes Google authentication authority.
...
- `app_organization_auth_policies`, retained as inert legacy compatibility data; its organization flag and row version no longer grant or remove Google authentication authority
...
- `app_auth_mutation_receipts`, deduplicating legacy policy updates and user identity-link commands without storing OAuth tokens or provider subjects in command results
...
An invited account is not a normal active account. Ordinary sign-in rejects invited, disabled, expired, or revoked access. Invitation activation consumes the code and invitation in one database transaction and activates the exact invited organization membership. Google cannot bypass that lifecycle: identity linking requires a recent, non-impersonated session, a matching verified email, a direct active membership, and a stable idempotency key; Google login requires the exact stored provider subject plus verified email, an active existing user, and a direct active membership, and never falls back to email-only authorization. Switching validates the selected direct membership again and rotates the session token. A linked identity cannot create users or memberships, inherit access from a root or parent organization, or authorize another user's Google login; linking is blocked while support impersonation is active. Settings and administration endpoints enforce both the global privilege and active membership boundary instead of trusting client-selected organization IDs. Dashboard defaults are saved only after access to the selected board or pipeline is resolved within the active workspace. Background agent workers use a separate bearer-secret channel with validated operator, organization, and board claims; they never create or reuse browser sessions.
