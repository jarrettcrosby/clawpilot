...
- Every active user membership receives one durable **ClawPilot User Guide** as the first document in that workspace. Dashboard and Docs reads provision or refresh it, and the scheduled document refresh keeps existing memberships current. The guide is personalized with the user's workspace, role, and relevant access guidance; it links every application module and explains project-agent, CRM/pipeline, Accounting/POS, permissions, mobile, session, and troubleshooting workflows.
