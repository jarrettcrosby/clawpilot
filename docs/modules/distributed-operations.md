...
Migrations `0192`, `0193`, and `0194` are a forward-only repair chain for that
...
zero-versus-nonzero invariant. It does not change data, provider authority, or
write behavior. The repair is required because PostgreSQL otherwise rejects an
otherwise-valid execution at deferred-constraint time with SQLSTATE `42601`
before the domain invariant can be evaluated.
