...
- a separate migration `0179` Active multi-package execution foundation derived from one exact immutable `0177` Shadow preparation. Preparing it requires the current organization-scoped Operations `active` revision and retains one single-warehouse shipment group, the exact Shadow package set, and one inherited Shadow planning estimate across every package. A whole-shipment carrier attempt is persisted before dispatch; a known failure may create a new consecutive attempt with a new idempotency identity, while prepared, succeeded, or unknown outcomes block another dispatch and an unknown outcome requires reconciliation. Success requires exactly one production label and shipment lineage for every prepared package. Database triggers reject production labels or shipments outside Operations Active and reject lineage that does not match the durable group attempt. This foundation adds no command route, worker, provider adapter call, automatic activation, packing-slip generation, inventory consumption, commerce fulfillment export, or channel write-back; those remain later executable slices and cannot be inferred from migration acceptance alone;
...
or provide provider-side audience enforcement.
...
The production-execution boundary now has two deliberately unexposed pure
contracts plus the append-only migration `0180` persistence authority.
...
`0180` itself invokes no carrier. The HTTP/token/network worker, shipment-label
dispatch and finalization, package-level tracking, final packing slips,
inventory consumption, commerce fulfillment, and Shopify/Faire writeback all
remain later executable slices. No route or worker currently exposes this
