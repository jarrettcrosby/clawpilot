...
- a separate migration `0179` Active multi-package execution foundation derived from one exact immutable `0177` Shadow preparation. Preparing it requires the current organization-scoped Operations `active` revision and retains one single-warehouse shipment group, the exact Shadow package set, and one carrier/service selection across every package. A whole-shipment carrier attempt is persisted before dispatch; a known failure may create a new consecutive attempt with a new idempotency identity, while prepared, succeeded, or unknown outcomes block another dispatch and an unknown outcome requires reconciliation. Success requires exactly one production label and shipment lineage for every prepared package. Database triggers reject production labels or shipments outside Operations Active and reject lineage that does not match the durable group attempt. This foundation adds no command route, worker, provider adapter call, automatic activation, packing-slip generation, inventory consumption, commerce fulfillment export, or channel write-back; those remain later executable slices and cannot be inferred from migration acceptance alone;
- unexposed pure execution-evidence prerequisites: `carrierWholeShipmentRateFoundation.ts` deterministically prepares bounded UPS/FedEx whole-shipment rate requests for explicitly selected sandbox or production endpoints and parses recorded responses without network I/O, while `activeCarrierDispatchSnapshot.ts` validates and deep-freezes one production dispatch snapshot binding the exact organization, order, plan, warehouse, Active carrier attempt, integration/carrier account, credential revision and fingerprints, billing relationship, origin, destination, successful production fulfillment rerate, single service, and ordered package set, and derives its provider idempotency identity. Recorded fixtures and contract tests prove deterministic request/response and snapshot invariants only. This slice adds no HTTP client, access-token acquisition, persistence producer, command route, worker, carrier shipment/label/void call, finalizer, tracking ingestion, packing-slip generation, inventory consumption, commerce-fulfillment export, or channel write-back. It does not make migration `0179` executable: append-only production fulfillment-rerate persistence and selection, dispatch claiming/finalization, and isolated PostgreSQL acceptance remain later slices;
...
- incomplete customer-supplied packaging-material drafts that retain only the supplied dimensions, explicit inner/outer/unconfirmed basis, evidence type/reference, and source while leaving unknown depth, tare, maximum weight, cost, currency, and stock null; activation fails closed until verified usable inner dimensions and every operating fact are present. A separate trusted-development, plan-first AG Alchemy command stages the four customer-supplied material drafts and six explicit provider sell-unit pack classes only after an administrator supplies exact `gp` Product Global IDs and an explicit active owner/admin actor. It verifies the Railway environment name and database identity, binds apply to the exact fresh plan fingerprint, projects every assigned Product's current exact channel state to the class's default pack version with retained provider source revision/hash, and fails if that provider evidence is absent. A complete channel source hash binds each mapping, so an image-only or other catalog revision still requires an explicit mapping version that carries the unchanged pack profile forward; a future unattended carry-forward may run only after proving that every pack-critical source fact is unchanged. Observation time alone is not mapping drift because an otherwise identical catalog read receives a new observation timestamp. Its title/SKU suggestions are discovery aids only. Guarded apply may replace the six exact untouched or legacy-equivalent synthetic starter drafts and their empty stock placeholders, but fails on partial, active, edited, referenced, or otherwise conflicting starter state. It never activates a material, profile, relationship, or recipe or infers intact-case inventory;
...
construction. The callback requires an exact, unexpired simulated Shadow
policy for the Shopify customer GID and requires every shippable line to use an
explicitly configured Test Product variant. The operator configures each
Shadow proof window from 15 through 240 minutes; the default is 60 minutes.
Saving or renewing starts a new bounded window. Missing configuration, an
expired policy, a missing or different customer ID, or any other shippable
...
test only binary allow or hide: an unexpired non-`hide_all` policy admits the
...
inside its unexpired Shadow window and the Test Product gate is present.
...
successful customer-neutral response. The live proof remains a bounded
operator test rather than a generally enabled production-store rate.
...
trusted-development acceptance for this boundary. It requires migrations
`0169` and `0171`, then applies only migration `0172` inside one transaction.
...
The next production-execution boundary now has two deliberately unexposed,
pure contracts. `carrierWholeShipmentRateFoundation.ts` prepares one complete,
ordered 1-50-package UPS or FedEx production-rate request and parses a recorded
...
unexpired successful production rerate, its exact carrier/account/credential
lineage, billing relationship, registered origin, ordered package set, selected
service, and server-derived retry identity into an immutable dispatch snapshot.
Neither contract obtains a token, opens an HTTP connection, persists a row,
leases a job, buys or voids postage, creates a label or shipment, produces a
tracking-bound packing slip, or writes to a commerce provider. No route or
worker exposes either contract.

The next schema slice must preserve migration `0177` as sandbox Shadow history
and add a separate append-only production fulfillment-rerate ledger. That
ledger must bind every request, response, offer selection, account and
credential revision, registered origin, destination, payer relationship,
ordered package version, and expiry before a carrier-create attempt may
reference it. Active fulfillment may select a different service or amount from
the Shadow estimate, but package and inventory conservation, current authority,
one carrier service for the complete shipment, and one server-derived provider
idempotency identity remain mandatory. Unknown carrier outcomes must reconcile
before retry. Until that persistence and isolated PostgreSQL acceptance exist,
the pure contracts are validation foundations rather than production dispatch
authority.
