...
This document remains the **target contract** for the full module. The current development slice includes operations migrations `0081` through `0094`, `0097` through `0101`, and `0107` through `0235`; a tenant-scoped order workbench; explicit idempotent single-warehouse fulfillment-plan acceptance, warehouse-release, bulk all-ready pick-confirmation, pack-verification, sandbox-label-create, and sandbox-label-void commands for eligible non-archived orders; a durable exception queue; scoped activation controls; canonical CRM catalog projection; provider-customer resolution; team-managed product/package imports; warehouse-scoped Packaging Materials management; versioned Product each/case profiles and separately evidenced exact-case and loose-each recipes; retained Shopify/Faire channel taxonomy; immutable CRM Product image assets; an exact resource-scoped Shadow Shopify image-publish command; organization-scoped direct carrier credential administration; UPS and FedEx sandbox rating with an account-derived origin and editable test destination; a separate rate-selected diagnostic label-create with customer-selected provider-native output, stored-label download/print, and void workflow; immutable provider-source bytes plus an explicit derivative-artifact provenance boundary; append-only redacted provider evidence; carrier-rate delegation; direct multi-account carrier CSV import; selected-batch GL Coding; separate financial review; billed-actual Triangle/Square/Circle settlement evidence; append-only settlement status transitions; capability-aware printer configuration; enrolled local print-agent delivery with controlled reprints, same-warehouse fallback, and agent-declared format/media/document capabilities; final shipment packing-slip and provisional exact-package Pack Work Instruction PDF renderers, immutable artifact-payload store, authenticated artifact stream, tracking-observation schema, and commerce-fulfillment export state model; a Shopify/Faire sales-channel control plane; a bounded development-only Shopify held-order preview; leased provider-read-only full-product-catalog reconciliation and current-order staging workers with a production-safe account fence; guarded development-only product-catalog mapping and operational-order workflows with durable pre-call read intents, resource-scoped encrypted continuations, first-class rejection dispositions, and canonical promotion; a bounded, leased, automatically scheduled, read-only Shopify inventory reconciliation for explicitly mapped eligible locations and warehouses with the manager command retained as manual recovery; a strict development-only Shopify cartonization preview; a recipe-first, assumption-watermarked sandbox package-and-rate evidence workflow; an executable development-only two-pass pack-and-rate replay workbench; an authenticated idempotent command that derives one immutable Active execution, shipment group, and ordered package set from exact zero-write Shadow evidence; append-only production fulfillment-rerate runs, exact package snapshots, prepared provider attempts, terminal results, normalized offers, one immutable selection, and authoritative Active dispatch linkage; an authenticated Operations command backed by a read-only production UPS/FedEx whole-shipment HTTP rerate executor that commits the exact prepared attempt before token/rate I/O and persists terminal response evidence; a separate authenticated manual command that selects one unexpired offer as the single service for that whole package set without provider I/O; a persistence-only Active dispatch-attempt boundary with one durable owner, exact replay, strictly proven retryable nonshipment retry, unknown-outcome blocking, and safe failed/unknown finalization; exact authority-gated Shopify and Faire fulfillment/tracking dispatchers with operator reconciliation and bounded scheduled recovery; and active-workspace measurement-presentation and product-currency defaults. The deterministic mock flow remains an internal automated-test harness only. Hosted mock generation is disabled and historical mock artifacts are archived. These features prove PostgreSQL authority and application boundaries; they do not establish historical closed-order commerce import, production or bidirectional provider inventory synchronization, production commerce mutation workers, production carrier mutation, service auto-selection, production label or tracking creation, carrier tracking ingestion, generic provider-agnostic commerce-export dispatch, continuous reverse catalog synchronization, pickup scheduling, accounting export, invoice/AR workflow, payment adapters, or production fulfillment-optimizer activation.
...
- exact post-commit Shopify and Faire fulfillment/tracking dispatchers plus operator reconciliation and bounded scheduled recovery. Execution remains gated by the exact provider account, credential generation, source identity, Operations Active capability claims, immutable shipment/package/tracking evidence, and runtime configuration. Shopify uses an explicit account notification policy with a reviewed per-order exception; sandbox E2E forces notification off. Faire retailer notification is provider-managed and has no ClawPilot override. These named adapters do not provide a generic commerce-export adapter or continuous reverse catalog synchronization;
...
- separate feature-gated Shopify/Faire catalog and operational-order workflows for configured and verified accounts: each provider read first persists a fixed idempotent intent, bounded product pages stage map/create/exclude candidates, and bounded current-window order pages retain every provider lifecycle state. Open demand may stage, resolve, validate, and promote; fulfilled, cancelled, or closed rows remain visible history and cannot create new ClawPilot fulfillment demand. Continuations are encrypted and resource scoped, Shopify pages are time-fenced, Faire order candidates require exact refresh, and every rejection exposes exact order retry or audited exclusion. The Operations Orders refresh discovers each running connected store, continues bounded provider pages, then reconciles existing canonical statuses; both canonical and provider-history lists use authenticated keyset paging instead of silently truncating at 100 rows. A leased provider-read-only order worker uses the same read/capture/normalize path, with production narrowed to active verified production accounts and current non-test staging without automatic customer resolution, promotion, exact refresh, canonical order creation, inventory mutation, cursor advance, or provider write;
...
- an order-specific **Enable training** control for an imported Shopify or Faire order while Operations is `shadow`. The commerce connection may be `sandbox` or `production`; a mock account is never eligible. The run seals the exact source order, candidate, account, credential generation, and Shadow revision, then advances only a separate local overlay through plan, release, simulated pick, simulated pack, and simulated completion. Provider order revisions continue to mirror on the canonical order and are shown as source drift without rewriting the frozen training facts. Training uses simulated order and material availability, so zero provider inventory, zero local inventory, zero packaging stock, an unmapped Shopify location, or a provider-managed Shopify fulfillment assignment does not prevent the exercise. It still requires active factual Product pack profiles, recipes, packaging-material dimensions, tare, and weight limits. Database guards keep canonical fulfillment plans, reservations, inventory ledger/positions, packaging claims, waves, picks, operational packages, shipments, labels, postage, billables, tracking, and commerce exports unchanged. Shopify/Faire provider-write, production-postage, inventory-mutation, and packaging-stock-mutation counters remain exactly zero. Reset preserves immutable prior-generation audit history; unresolved sandbox-label evidence blocks reset rather than permitting a blind retry. The separate Shipping Settings sandbox account/label/printer diagnostic is not an order-bound training label;
- a feature-gated Shopify inventory reconciliation for a configured and verified account with `read_inventory`, `read_locations`, and `read_products`: a leased worker refreshes every enabled exact Shopify-location mapping before checkout evidence reaches half its configured maximum age, while the manager command remains a recovery control. A manager may map an eligible merchant-managed Shopify location to an existing warehouse/location or atomically create the standard ClawPilot warehouse topology from the freshly read Shopify address and route inventory to `RESERVE-01`. Every read, prior-run comparison, stale-zero operation, queue job, and watermark acknowledgement remains mapping-scoped. Provider/app-managed fulfillment-service locations are visible but read-only. These paths retain all eight requested Shopify quantity states and operational product facts, project only exact product mappings into Shopify-authoritative Operations balances, and perform no provider write, order mutation, shipment creation, or fulfillment export;
...
- an Active-only **Reopen for replanning** operational correction for a connected, current-revision Shopify or Faire order whose one accepted plan remains `planned` and unreleased. The server projects the exact plan generation, order row version, and a fingerprint of every compensating fact; web and iPhone managers must submit those same fences with a reason, stable idempotency key, and a second confirmation. The transaction releases only ClawPilot-local reservations, local reserved-balance ledger effects, provider-commitment claim records, and packaging claims; cancels the prior plan; returns the canonical order to `imported`; and appends immutable correction, domain-event, audit, and receipt evidence. It makes zero Shopify/Faire or carrier calls and never edits provider-authored order facts. The next accepted plan uses `MAX(version_number) + 1`, so a cancelled version 1 is followed by version 2 and cannot be overwritten or reused. Released, picking, packed, labeled, shipped, fulfilled, native one-off, and mock orders are ineligible. Released work stays visibly blocked until a separate durable picker-device queue lease, recall, acknowledgement, and expiry workflow can prove every offline copy has been withdrawn; server-side absence of pick evidence alone is not treated as physical rollback authority;
- a separate migration `0179` Active multi-package execution foundation derived from one exact immutable `0177` Shadow preparation. Preparing it requires the current organization-scoped Operations `active` revision and retains one single-warehouse shipment group, the exact Shadow package set, and one inherited Shadow planning estimate across every package. A whole-shipment carrier attempt is persisted before dispatch; under the current `0183` through `0187` safety layer, only a failed attempt carrying retryable persisted diagnostic proof of noncreation may create a new consecutive attempt with a new idempotency identity, while prepared, succeeded, or unknown outcomes block another dispatch and an unknown outcome requires reconciliation. Success requires exactly one production label and shipment lineage for every prepared package. Database triggers reject production labels or shipments outside Operations Active and reject lineage that does not match the durable group attempt. This foundation itself adds no command route, worker, provider adapter call, automatic activation, packing-slip generation, inventory consumption, commerce fulfillment export, or channel write-back; the later gated Shopify/Faire shipment-completion and dispatcher slices described below provide only their exact named paths, which cannot be inferred from `0179` migration acceptance alone;
...
Production commerce mutation remains out of scope until later delivery gates verify provider credentials, processed webhook and polling imports, canonical mappings, provider attempts, reconciliation/replay, complete operational health, recovery commands, and an explicitly approved integration and warehouse cohort. Enabling Shopify signed receipt intake still accepts only its enumerated receipt topics; order and customer webhook topics remain rejected until their retention/privacy lifecycle and canonical processor exist. The separately gated held-order preview remains sandbox/development-only and authorizes no provider write. A verified product-readable Shopify or Faire connection authorizes the catalog worker without a second approval; production additionally requires the account to be active, verified, and production. The catalog worker may create ClawPilot product masters and exact mappings but never reads or changes orders or inventory and never writes to the provider. A readable production account may also stage current non-test order candidates and rejections, but production skips automatic customer creation, automatic order promotion, promotion-driven Faire exact refresh, and canonical-order creation. Development retains its explicit automatic-promotion cohorts. The Shopify inventory reconciliation and Faire listing-observation workers are production-capable only for active verified production accounts and remain provider-read-only; neither performs bidirectional provider inventory synchronization. Sandbox rating and label evidence do not authorize production rating, label creation, pickup scheduling, shipment confirmation, or inventory consumption. `app_documents` is not used as a substitute for logistics artifacts.
...
The development-store callback has a paired checkout execution boundary. One
registered Shopify CarrierService may retain one through eight selected direct
UPS/FedEx sandbox billing accounts for TEST and one through eight selected
direct UPS/FedEx production billing accounts for LIVE. Operations `shadow`
executes only the complete TEST group; Operations `active` executes only the
complete LIVE group. The opposite group may coexist but is never mixed into a
request. Shadow-to-Active authorization verifies the future LIVE group before
the state transition. LIVE checkout execution is still a read-only carrier
rate operation: it cannot buy postage, create a label, tender a shipment, or
authorize any other production mutation. Receipt reuse is fenced by the
...
| Catalog/inventory/orders | GraphQL capabilities are scope-dependent, including protected/restricted scopes | Public brand scopes cover product, inventory, order, shipment, retailer, and review reads/writes as documented | Separate bounded catalog reads reconcile provider product/variant mappings; separate bounded order reads stage nonterminal orders. Production accepts active verified production accounts only and excludes Shopify test orders. Shopify inventory and Faire listing observations are read-only; the CRM catalog remains authoritative, closed history is not imported, and no refresh path writes a provider |
...
Migration `0124` added the original Shopify inventory boundary. Migration `0288` expands that boundary to explicit multi-location routing without granting a provider write. The runtime requires Postgres, the commerce-intake feature gate, a configured and verified Shopify account, and effective `read_inventory`, `read_locations`, and `read_products` grants. Every enabled merchant-managed Shopify location must resolve through one exact mapping to an active ClawPilot warehouse, internal inventory location, pool, provider-location identity, and mapping row version. Production additionally requires the account to be active and production. The mapping command can bind an existing warehouse/location or atomically create a local warehouse and its standard 13-location topology from a freshly verified Shopify address; that local command records `providerWrites = 0`. Provider/app-managed fulfillment-service locations remain discovery-only. The mapped queue uses a separate rolling-safe lifecycle, serializes provider reads per account, and acknowledges the account watermark only after all current mapped locations succeed. The manager command and automatic worker share this exact read-only reconciliation boundary; neither can create or edit a Shopify location, select an arbitrary provider host, register a webhook, advance an order cursor, or make a Shopify mutation. Before cartonization, a separate read-only preflight resolves Shopify's current fulfillment-order assignment to the exact mapping and preselects that warehouse; unmapped, split, closed, or provider-managed assignments remain blocked by the final authority check.

Migration `0289` adds a distinct Shopify merchant-location administration boundary; it does not weaken the `0288` import path. Only the exact enabled Railway development runtime and allowlisted verified Shopify Partner development-store account can expose it. An owner or authorized administrator may prepare and then explicitly execute one `locationAdd`, `locationEdit`, or `locationActivate` against an eligible merchant-managed location after exact scope, identity, warehouse, mapping, row-version, address, and typed-confirmation checks. The short-lived authorization is claimed before provider I/O, and the adapter reads back the result. Processing or unknown outcomes never auto-retry and require positive reconciliation. Fulfillment-service locations, location deactivation/deletion, inventory activation or quantity writes, and automatic routing of a newly added Shopify location remain unavailable.
...
bindings, and one through eight selected direct UPS/FedEx sandbox billing accounts. Multiple
accounts from the same carrier remain distinct through exact account identity. Carrier reads occur
...
The full interactive intake and decision implementation remains development-only behind `CLAWPILOT_COMMERCE_INTAKE_ENABLED=1`. The same flag now also enables the narrower hosted production read/reconciliation workers, but only for active, verified production Shopify or Faire accounts. Production workers may reconcile catalogs, stage current non-test orders, import provider product images, and run the separately bounded Shopify and Faire inventory observation lanes. Production order reconciliation skips automatic customer creation, automatic order promotion, promotion-driven Faire exact refresh, canonical-order writes, provider writes, and inventory writes. Sandbox accounts and Shopify test orders remain development-only. A product-readable eligible connection authorizes catalog sync without a second approval and initializes a missing policy; the leased job still requires current scope, credential generation, policy revision, and a `shadow` or `active` Operations product target. These code contracts do not by themselves claim hosted provider acceptance; deployment health and account-specific provider evidence remain required.
...
Exact-artifact download requires no workstation installation. Silent cloud-to-local delivery requires an enrolled local agent or an approved managed printing bridge on a computer or print server that can reach the device. The bundled agent sends raw ZPL directly to a compatible network Zebra without an operating-system driver. It advertises carrier labels on 4 x 6 or 4 x 8 media plus product and location barcode labels on 2 x 1, 3 x 1, 4 x 2, 4 x 6, or 4 x 8 media. PDF/PNG and USB/operating-system queues require a separately implemented backend with truthful capabilities. ClawPilot does not import arbitrary GitHub printer-driver collections; every backend must be version-pinned, licensed, security-reviewed, and certified for its declared format, DPI, media, copy count, and device class.
...
Each adapter declares accounts, services, negotiated/published rates, transit estimates, address validation, labels, voids, tracking, manifests, pickups, customs, proof of delivery, idempotency, and reconciliation capabilities. USPS, UPS, and FedEx are initial direct targets. RocketShipIt is an optional aggregator behind the same canonical boundary, not a replacement shipment master. Worldwide Express is modeled as the API provider/broker: its Small Parcel offers execute through UPS and each LTL offer retains the actual returned carrier and SCAC. R+L is a separate direct LTL provider. Provider, transport mode, executing carrier, provider account, and handling-unit plan are independent canonical facts. Small Parcel rates bind loose cartons/poly bags; LTL rates bind versioned outbound pallets containing those cartons and must not reuse a product pack-level pallet as shipment evidence. An exact-pallet density result is advisory until the operator confirms full-density eligibility and no mixed-commodity, handling, stowability, or liability exception; immutable `gfca` evidence then binds the exact pallet dimensions, gross weight, class, optional independently verified NMFC item/subitem, and source. Freight tender succeeds through shipment/BOL/PRO/pickup evidence rather than a fabricated label per carton. Read-only WWEX sandbox and R+L production rate capabilities can be activated independently after live credential verification, but all pickup/BOL/label/tender capabilities remain disabled until the one-off orchestration, document retention, billing facts, and reconciliation gates are wired and certified. Activated carrier orchestration is required to use controlled parallelism, per-provider timeout and circuit-breaker policy, bounded response storage, exact account/service snapshots, canonical millimeters/grams, and exact money conversion. The [small parcel carrier adapter architecture](../architecture/small-parcel-carrier-adapters.md) defines the direct parcel boundary. The [brokered parcel and LTL carrier architecture](../architecture/brokered-parcel-and-ltl-carrier-adapters.md) defines WWEX/R+L identity, classification, palletization, freight tender safety, and certification gates.
