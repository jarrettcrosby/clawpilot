...
This is a development-only implementation behind `CLAWPILOT_COMMERCE_INTAKE_ENABLED=1`; the runtime refuses production and `shadow` or `active` organization activation gates every read and decision. These contracts do not claim a successful hosted Shopify or Faire read, catalog mapping, rejection recovery, or order promotion until separate development deployment and browser acceptance evidence is recorded.
