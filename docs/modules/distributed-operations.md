...
Migration `0124` added the original Shopify inventory boundary. The current runtime requires Postgres, the commerce-intake feature gate, a configured and verified Shopify account, the effective `read_inventory`, `read_locations`, and `read_products` grant, one active ClawPilot warehouse, and one uniquely eligible active physical Shopify location. Production additionally requires the account to be active and production. An existing location mapping must still resolve to that same eligible provider location. The manager command and automatic worker share this exact read-only reconciliation boundary; neither can create a warehouse, select an arbitrary provider host, register a webhook, advance an order cursor, or make a Shopify mutation.
