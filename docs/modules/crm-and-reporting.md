...
Meeting time synchronization is bidirectional among the ClawPilot CRM surface, native SuiteCRM, and the signed-in user's selected Google Calendar. Saving a non-cancelled meeting queues an idempotent Calendar create or update; new events request a unique Google Meet conference. Calendar polling correlates events by provider event ID or the private `clawpilotMeetingReference`, applies changed time and event fields to the Postgres/SuiteCRM projection, and ignores provider echoes whose meaningful fields already match. Native SuiteCRM polling applies changed meeting time, subject, status, location, and description to Postgres and queues the corresponding Calendar update. Cancellation in Google or native SuiteCRM marks the CRM meeting cancelled; a later Calendar restore returns it to scheduled.
