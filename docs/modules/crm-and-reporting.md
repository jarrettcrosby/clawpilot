...
The interaction editor requires a controlled interaction type and offers only active ClawPilot users who can access the selected pipeline as the Agent. Administrators can map each ClawPilot user to one active native SuiteCRM username; the gateway then assigns the Note to that SuiteCRM user while preserving the ClawPilot email as the durable actor identity. Contact selection is a searchable multi-select scoped to the selected Organization and is optional only when the interaction is genuinely account-level. ClawPilot persists the complete ordered Contact set in a tenant-safe relationship table and keeps the first selected Contact as the primary compatibility relationship used by the current SuiteCRM Note projection.
