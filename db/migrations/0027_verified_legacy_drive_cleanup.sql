-- Re-run legacy hierarchy cleanup with verified Drive child responses. The v4
-- target prevents outgoing workers from claiming this item during rollout.
...
SELECT
  ...
  'google_workspace_v4',
  jsonb_build_object('pipelineId', pipeline.id::text, 'layoutVersion', 4),
  'queued',
  0,
  'pipeline:' || pipeline.id::text || ':hierarchy:v4',
  now(),
  now(),
  now()
