-- Shopify CarrierService rating can use either one configured carrier or two.
...
-- The carrier binding table already enforces one row per provider and one row
-- per carrier account; this migration changes only the required set size.
...
COMMENT ON FUNCTION
  ...
  'Returns true only when the active sandbox Shopify commerce account, provider-verified bounded store entity, activation, warehouse, current packaging stock, and one or two selected unique verified UPS/FedEx bindings are ready.';
