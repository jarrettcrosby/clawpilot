-- Preserve legacy single-package cartonization evidence while making every new
-- comparison a pair of whole-shipment reads: one UPS Shop request and one
-- FedEx multi-piece shipment request.
...
COMMENT ON COLUMN operations_carrier_rate_requests.purpose IS
  'sandbox_rate_test is the fixed credential diagnostic; cartonization_package_rate is retained legacy single-package evidence; cartonization_shipment_rate is one ordered multi-package request for an entire cartonization plan.';

ALTER TABLE operations_cartonization_rate_evidence_quotes
  DROP CONSTRAINT IF EXISTS
    operations_cartonization_rate_evidence_quotes_rate_purpose_check,
  ALTER COLUMN rate_purpose
...
COMMENT ON COLUMN
  operations_cartonization_rate_evidence_quotes.package_rate_context_hash IS
  'Purpose-aware immutable context hash: legacy package context for cartonization_package_rate or shared ordered shipment context for cartonization_shipment_rate.';
...
BEGIN
  ...
  IF package_count NOT BETWEEN 1 AND 50 OR EXISTS (
    ...
    RAISE EXCEPTION
      'Cartonization rate evidence requires 1-50 packages and one UPS and one FedEx supporting edge per package';
  END IF;
  ...
  IF shipment_scope THEN
    IF (
      ...
      RAISE EXCEPTION
        'Cartonization shipment evidence requires exactly one shared carrier result per provider';
    END IF;
...
COMMENT ON FUNCTION
  ...
  'Deferred immutable validation for legacy package evidence or 1-50 ordered packages supported by exactly one UPS and one FedEx whole-shipment result.';
