-- Automatic development-only Shopify intake needs to establish that a held
-- order has exactly one current checkout quote family before it performs any
-- workflow resolution. The authoritative reconciliation functions introduced
-- in 0189 intentionally require a promoted candidate and canonical order, so
-- retain that post-promotion fence and expose a separate read-only preflight
-- over the same immutable receipt facts.

CREATE OR REPLACE FUNCTION
  operations_shopify_checkout_rate_match_candidate_facts_for_workflow(
    requested_organization_id uuid,
    requested_order_candidate_id uuid,
    enforce_reconciliation_deadline boolean,
...
AS $$
  ...
  WHERE candidate.organization_id = requested_organization_id
    ...
    AND (
      NOT require_promoted_candidate
      OR (
...
COMMENT ON FUNCTION
  ...
  'Shared immutable Shopify checkout quote facts. Promoted-only callers retain the authoritative canonical-order fence; the explicit false mode is reserved for read-only preflight.';

COMMENT ON FUNCTION
  operations_shopify_checkout_rate_preflight_match_candidates(
    uuid, uuid, boolean
  ) IS
  'One deterministic checkout quote-family representative for a held or ready Shopify candidate. This is a read-only eligibility preflight; canonical reconciliation must still repeat the promoted-only match in its transaction.';
