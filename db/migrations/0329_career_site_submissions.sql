CREATE TABLE career_site_submissions (
...
CREATE INDEX idx_career_site_submissions_owner_created
...
CREATE INDEX idx_career_site_submissions_form_created
  ON career_site_submissions (form_type, created_at DESC, id DESC);

CREATE TABLE career_site_submission_outbox (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  CONSTRAINT career_site_submission_outbox_requeue_count_valid CHECK (
    requeue_count BETWEEN 0 AND 3
  ),
  CONSTRAINT career_site_submission_outbox_requeue_audit_valid CHECK (
    (
      requeue_count = 0
...
CREATE INDEX idx_career_site_submission_outbox_due
