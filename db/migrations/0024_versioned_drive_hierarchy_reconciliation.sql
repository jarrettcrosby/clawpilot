-- Queue hierarchy reconciliation on a target that pre-v2 workers cannot claim.
-- This prevents the outgoing worker from acknowledging the migration with the
-- legacy Users/<email> folder layout during a rolling deployment.
...
SELECT
  ...
  'google_workspace_v2',
  jsonb_build_object('pipelineId', pipeline.id::text, 'layoutVersion', 2),
  'queued',
  0,
  'pipeline:' || pipeline.id::text || ':hierarchy:v2',
  now(),
  now(),
  now()
